...
def prepare(root: Path, assets: Path) -> dict:
    ...
    reference = root / "tests/fixtures/camera_goal_transport/reference-top.jpg"
    ...
    backend = {"schema": "ugrp.rgb_skill_backend.v1", "map_id": "dispatch_open", "seed": 11,
               ...
               "max_commands": 6000, "grasp_model_dir": "outputs/study-inputs/assets/grasp",
               "stage_model_dir": "outputs/study-inputs/assets/varied",
               "reference_top": "outputs/study-inputs/assets/reference-top.jpg"}
    ...
    assignments = [{"scenario_id": kind + "-physical-replay", "parent_map_id": "dispatch_open",
    ...
    environment_ref = record("environment.json", {
        ...
        "review": review_ref, "provenance_artifacts": {
            "data": [ref("assets/" + d + "/training-report.json") for d in ("grasp", "varied")],
            "checkpoint": [ref("assets/grasp/student-skill.json"), ref("assets/varied/varied-start-skill.json")],
            "prompt": [prompt_ref]}})
    ...
    final_config = {"stage": "physical_replay", "claim_scope": "development_connection_smoke",
        "submitter": "D2", "backend_id": descriptor["backend_id"], "backend": backend,
        ...
        "trials": [{"run_id": kind + "-physical-replay", "scenario_id": kind + "-physical-replay",
    ...
    return {"source_sha": state["git_sha"], "output": str(output), "ready": False,
            "requires": ["independent A2 offline boundary review", "source-bound E0 preflight"],
...
def main():
    ...
    print(json.dumps(prepare(ROOT, args.assets_root), ensure_ascii=False))
