...
- [2026-09-26 자기 카메라 폐루프 문 통과 TensorBoard 스냅샷](2026-09-26-tb-owncam-loop/README.md): PR #178 코호트 22회 전체(실패 포함)를 새 스냅샷 `0926-zone-owncam-loop` 22 run으로 변환했다. dev-amended(a1–a3) 10 · dev-frozen(a4, 보고 dev) 6 · test-preregistered 6을 run 이름과 `case` 열로 구분하고, 학생 SIM 시간·발행 명령·도착 선언(20/22)과 게이트 통과(17/22)를 분리해 표시한다. scalar 154개와 Text 값 198개를 원본 `result.json`·`manifest.json`에서 재계산해 대조(불일치 0), 공용 서버(PID 9291/9293) 재시작 없이 22 run 표시·값 일치 확인. 새 실험·SIM·모델 호출은 없고 분할·조건·코호트를 합산하지 않는다.
