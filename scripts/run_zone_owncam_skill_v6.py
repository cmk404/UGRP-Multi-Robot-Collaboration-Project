#!/usr/bin/env python3
"""Runner v6 for ``wrist_zone_skill_v6`` (head-on face yaw, probe re-anchor hook, static keep-outs). DIAGNOSTIC.

A new runner (``zone_owncam_runner_v6``) so runner v5 stays the recorded source of
its cohort (541-548). Changes against scripts/run_zone_owncam_skill_v5.py:

* skill ``wrist_zone_skill_v6`` (top-edge own-RGB face yaw; public
  ``reanchor_after_probe``; static keep-out approach point + guard);
* scenarios with axis-aligned boxes (yaw 0 and +/-2 deg) besides rotated ones
  (PR #201 dev-a2: v5 never became face-ready on axis-aligned boxes);
* idle peers as static layout keep-outs: r2/r3 stand at their spawn spots, or
  r2 is parked at a fixed spot next to the bay (``parked_r2``, relative to v5's
  approach point). Those spots are static-layout input (``StaticKeepout``) for
  the skill and the path planner; live peer poses are never read by the
  controller;
* evaluation-only ``r1_peer`` contact class on every physics step.

Diagnostic like v5: the pose source is the GT stub, so ``m1_success`` is
always False; the skill-isolation outcome is ``diagnostic_success``.
"""
...
RUNNER = 'zone_owncam_runner_v6'
SCHEMA = 'ugrp.zone_owncam_skill.v6'
...
SCENARIOS = {
    ...
    412: {'start': (2.60, -1.50, 0.00), 'bay': 'E2', 'offset': (.05, -.06), 'yaw_deg': 0., 'slot': 'B2'},
    413: {'start': (2.55, 0.40, 0.05), 'bay': 'E3', 'offset': (-.08, .06), 'yaw_deg': 2., 'slot': 'A1',
          'parked_r2': (0., -.30)},
    414: {'start': (2.60, -2.40, 0.00), 'bay': 'E1', 'offset': (.10, .05), 'yaw_deg': -30., 'slot': 'C3',
          'parked_r2': (0., 0.)},
    # test (pre-registered in experiments/2026-09-25-zone-owncam-skill/README.md before any v6 test run)
    551: {'start': (2.55, -2.30, 0.00), 'bay': 'E1', 'offset': (.08, .05), 'yaw_deg': 0., 'slot': 'B1'},
    552: {'start': (2.60, -1.20, 0.05), 'bay': 'E2', 'offset': (-.10, .08), 'yaw_deg': 0., 'slot': 'A2',
          'parked_r2': (0., .30)},
    553: {'start': (2.50, 0.30, 0.00), 'bay': 'E3', 'offset': (.06, -.10), 'yaw_deg': 2., 'slot': 'C1'},
    554: {'start': (2.60, 1.00, -0.05), 'bay': 'E4', 'offset': (-.05, .05), 'yaw_deg': -2., 'slot': 'B3'},
    555: {'start': (2.70, -2.60, 0.10), 'bay': 'E1', 'offset': (-.12, -.10), 'yaw_deg': 0., 'slot': 'A3'},
    556: {'start': (2.55, -1.70, 0.00), 'bay': 'E2', 'offset': (.12, -.04), 'yaw_deg': 30., 'slot': 'C2'},
    557: {'start': (2.60, 0.20, 0.00), 'bay': 'E3', 'offset': (-.06, .12), 'yaw_deg': -30., 'slot': 'B2',
          'parked_r2': (0., 0.)},
    558: {'start': (2.65, 0.80, 0.00), 'bay': 'E4', 'offset': (.10, -.08), 'yaw_deg': 10., 'slot': 'A1'},
    559: {'start': (2.60, -2.10, -0.10), 'bay': 'E1', 'offset': (.04, .12), 'yaw_deg': -15., 'slot': 'C3'},
    560: {'start': (2.55, -1.40, 0.15), 'bay': 'E2', 'offset': (.00, .00), 'yaw_deg': 0., 'slot': 'B1'},
}
DEV_SEEDS = (412, 413, 414)
TEST_SEEDS = tuple(range(551, 561))
DEPENDENCIES = (
    'scripts/run_zone_owncam_skill_v6.py', 'scripts/run_zone_owncam_skill_v5.py', 'harness/m1_contract.py', 'harness/wrist_zone_skill.py',
    'harness/wrist_zone_skill_v2.py', 'harness/wrist_zone_skill_v3.py', 'harness/wrist_zone_skill_v4.py',
    'harness/wrist_zone_skill_v5.py', 'harness/wrist_zone_skill_v6.py', 'harness/visual_box_skill.py', 'harness/visual_box_surface.py',
...
def static_keepouts(seed):
    ...
    robot assignment) and, in ``parked_r2`` scenarios, the parking spot P1 next to the bay."""
    ...
    if 'parked_r2' in scenario:
        cx, cy = BAYS[scenario['bay']]
        ax, ay = cx - BAY_HALF_M[0] - v5.BAY_APPROACH_CLEARANCE_M, cy      # v5's approach point
        keepouts.append(v6.StaticKeepout('parking_P1', (round(ax + scenario['parked_r2'][0], 4),
                                                         round(ay + scenario['parked_r2'][1], 4)),
...
def build(seed, contact_profile):
    ...
    if 'parked_r2' in scenario:
        (px, py), = [k.xy_m for k in keepouts if k.keepout_id == 'parking_P1']
        world.robot('r2').set_base_pose_for_test((px, py, .032355118817659255), 0.)
    ...
    setup_only = {'box_xy_m': [bx, by], 'box_yaw_deg': scenario['yaw_deg'], 'bay': scenario['bay'],
                  ...
                  'parked_r2_rel_v5_approach_m': scenario.get('parked_r2'),
...
def make_planner(static, order, keepouts=()):
    ...
    def planner(start, goal, carrying):
        ...
        discs = [] if carrying else [(cx, cy, max(hx, hy))]
...
def input_contract(pose_label, static_map):
    ...
    return {'camera': 'robot_cam only; every observation strictly validated (N7 _validate_observation)',
            ...
            'runner': RUNNER, 'skill_profile': v6.PROFILE}
...
def main():
    ...
    skill = v6.WristZoneDeliveryV6(order, mode=args.mode, static_keepouts=keepouts,
    ...
    result = {
        'schema': SCHEMA, 'runner': RUNNER, 'profile': v6.PROFILE, 'seed': args.seed,
        ...
        'claim_scope': ('DIAGNOSTIC skill isolation on the open east section of zone_wide_door with a GT pose stub; '
                        'not M1, not own-camera localisation, no door crossing') if args.mode == 'diagnostic' else 'M1',
    ...
    print(json.dumps({'seed': args.seed, 'reason': reason, 'sim_s': result['sim_seconds'],
                      ...
                      'approach': (skill.approach_choice or {}).get('side')}))
