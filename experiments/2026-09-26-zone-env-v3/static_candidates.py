...
def make_candidate_world(sc):
    ...
    static['landmarks']['tags'] = candidate_tags(static)
...
def main(argv=None):
    ...
    scene, world, static = make_candidate_world(sc)
    tags = {t['id']: t for t in static['landmarks']['tags']}
        ...
        for group, spec_ in GROUPS.items():
            ...
            for bx, by in spec_['bases']:
                for view, key, pan in [('drive', drive, None)] + [(f'look_{q}', look, q) for q in pans]:
                    ...
                    name = f'{group}_x{bx:.2f}_y{by:+.2f}_{view}'
                    if view in ('drive', 'look_1500', 'look_970', 'look_2030') and by in (-.8, .05):
                        (a.output/'frames'/f'{name}.jpg').write_bytes(jpeg)
                    dets = sc.detect_frame(detector, seen, tags)
                    for d in dets:
                        d['true_range_m'] = round(float(math.dist(origin, tags[d['id']]['center_m'])), 4)
                    rows.append({'group': group, 'base': [bx, by], 'view': view, 'camera_z_m': round(float(origin[2]), 4),
                                 'detections': dets, 'frame_sha256': hashlib.sha256(jpeg).hexdigest()})
    finally:
        world.close()
    ...
    raw = {'schema': 'ugrp.zone_env_v3.st6_candidates.v1', 'posthoc': True,
           ...
           'candidates': {'count': len(tags), 'spacing_m': SPACING_M, 'heights_m': list(HEIGHTS_M),
    ...
    for group, s in summary.items():
        ...
        for e in s['positions'][:12]:
            print('   ', e['stations_seen'], e['position'], 'z', e['z_seen'], 'err', e['max_abs_range_err_m'], e['near_v3_site'])
