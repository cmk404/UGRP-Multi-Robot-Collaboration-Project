...
## Colab 회수 오류와 실행 수명 분리

- 이전 회수기는 FileNotFoundError 3회 뒤 실패했고, 소유 제어기의 finally가 실행 세션까지 종료했다. 최초 파일 오류의 원인은 확정되지 않았다.
- 같은 런타임의 전송 인증 갱신, 제한 시간 내 재조회, 회수·실험 종료 조건 분리를 구현했다. 실제 Colab 읽기 전용 잘못된 인증 검사에서 같은 FileNotFoundError를 재현하고 같은 endpoint의 새 인증으로 복구했다. 이는 최초 사건이 토큰 만료였다는 증명은 아니다.
- 새 제어기 `8ff2cd9`는 actor 소스 `5a51a355`를 그대로 실행한다. 성공·실패가 확정된 기존 37회는 해시와 조건을 검증해 보존하고, 미완료 194회만 새 런타임에서 수행한다. 재개 후 첫 2회 완료·원본 체크포인트 회수·내부 해시·TensorBoard 이벤트·영상 Range 응답과 화면을 확인했다. 전체 비교는 아직 진행 중이다.
- 전체 1,226개 테스트, 191개 하위 검사 통과(3개 건너뜀). `independent-recovery/colab-collection-repair.json`에 출처·검증 범위·실패 및 대시보드 근거를 보존했다. 인증정보나 미회수 결과를 저장소에 넣지 않았다.
