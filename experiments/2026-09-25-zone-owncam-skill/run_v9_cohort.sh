#!/bin/zsh
# v9 pre-registered cohort: arm P 601-610, runner v9, diagnostic mode (gt_stub pose, NOT M1),
# cargo_noslip_v1 (pending user decision), 2 lanes. Stop rule: any false OWN_RGB_PLACEMENT_IN_SLOT stops it.
set -u
WT=${V9_WT:-/Users/changmin/projects/ugrp-wt/zone-owncam-skill-v9-frozen}   # detached worktree at the freeze commit (no dev edits can dirty it)
cd $WT
SHA=$(git rev-parse --short HEAD)
OUT=/Users/changmin/projects/ugrp/outputs/zone-owncam-skill-20260925/cohort-v9-$SHA
mkdir -p $OUT
LOG=$OUT/cohort.log
...
run1() {
  ...
  python3 scripts/run_zone_owncam_skill_v9.py --mode diagnostic --seed $seed --contact-profile cargo_noslip_v1 "$@" \
...
echo "$(date -u +%FT%TZ) cohort v9 source=$SHA dirty=$(git status --porcelain | wc -l | tr -d ' ')" >> $LOG
( for s in 601 603 605 607 609; do run1 P $s; done ) &
( for s in 602 604 606 608 610; do run1 P $s; done ) &
wait
