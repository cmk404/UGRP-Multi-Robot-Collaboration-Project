...
접촉 설정과 제어기까지 무조건 하나의 기본값으로 합치지는 않는다. 과거 `local_contact_fine` 성공과 `legacy` adapter 진단은 서로 다른 실험 조건이다. 실행 시 선택한 프로필과 실제 적용값을 기록하고 [실행 번들 버전](execution_versioning.md)으로 검증한다. 직전 공통 RGB 후보 v6는 영상에 맞춘 공동 역할 배정과 사전 검사를 물리 재생에서 확인했다. 단독 상자 대상은 성공했으나 공동 빔은 접근 단계 횟수 제한으로 파지 전에 끝났다. 현재 v7은 영상 진전이 있을 때만 접근 예산을 유한하게 연장하는 미검증 후보다. 공동 완주와 두 물체의 전체 임무는 아직 검증되지 않았다. 이전 번들 JSON은 변경하지 않고 원래 소스로 재현한다.
