...
- **실행 중:** 빔 운반 로봇 둘의 접근이 지원된 이유로 실패하면 두 로봇이 멈추고 대화한다. 지원된 이유는 거친 접근 한도, 미세 정렬 지원 범위 이탈, 도킹 실패다. 둘이 자기 영상과 실패 이유를 보고 `retry` 또는 `abort`를 만장일치로 정한다. `retry`는 짧게 뒤로 물러난 뒤 영상 접근을 처음부터 다시 하며, 최대 `--approach-retries`회(기본 2)다. 의견이 갈리면 멈춘다.
- **범위:** 동기 실행 전용이며 저장 계획 재생과 함께 쓸 수 없다. 파지·운반·상자 로봇의 실패는 아직 대화 대상이 아니다. `--diagnostic-fail-approach-once`는 대화·재시도 경로를 확인하려고 첫 접근 뒤 실패를 일부러 넣는 진단이다. 기록은 `result.json`의 `coordination`과 `team/conversation.jsonl`에 남는다.

자연어는 **실행 전 계획 지시**다. 현재 계약은 기존 beam 1개·box 1개, 로봇 3대, dock_a/b, north/south 경로다. 자유로운 새 작업이나 임의 맵에 필요한 스킬을 자동으로 만드는 기능은 없다. 목적지처럼 반드시 지켜야 하는 조건은 `--required-dock`으로도 지정한다. 실행 중 새 자연어 지시로 이미 승인된 plan을 바꾸는 기능은 아직 없다(동적 협업의 실패 대화는 retry/abort만 정한다). 새 작업은 종료 후 다시 실행한다. 저장된 plan 재생에는 새 `--task`를 함께 넣을 수 없다.
