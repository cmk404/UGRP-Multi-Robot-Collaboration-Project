...
def audit(root,models):
 ...
 for kind in ('spacing','endurance'):
  actors={rid:(PairGraspSpacing(r['map'],rid) if kind=='spacing' else EnduranceActor(r['map'],rid,r['mode'],slip_guard=r.get('slip_guard',False))) for rid in ROBOTS}
 ...
 close=[row for row in trace if row['stage']=='grasp_close' and not row.get('recovery_index')];skill=json.loads((models/'student-skill.json').read_text())
 ...
 recovery_audit=audit_recoveries(root,r,models,seen)
 ...
 recovery_evaluation=evaluate_recoveries(rows,r)
 if 'recovery_evaluation' in r and not _same(recovery_evaluation,r['recovery_evaluation']):raise ValueError('recovery output verdict mismatch')
 ...
 audit_contact_profile(_safe_file(root,'scene.xml'),r)
 return {'passed':True,'source_sha':r['source_sha'],'spacing_rounds':len(r['spacing_steps']),'endurance_rounds':len(r['steps']),'grasp':grasp,'recoveries':recovery_audit,'recovery_evaluation':recovery_evaluation,'physical_success':r['success'],'scope':'Exact saved RGB/command replay and output-only scoring; not a physical rerun'}
def main():
