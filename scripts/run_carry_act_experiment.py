...
import argparse,concurrent.futures,hashlib,json,shutil,subprocess,sys,time
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]

def write(p,v):p.write_text(json.dumps(v,indent=2)+'\n')
def main():
    p=argparse.ArgumentParser();p.add_argument('--out',type=Path,required=True);p.add_argument('--act-python',type=Path,required=True);p.add_argument('--mjpython',type=Path,required=True);p.add_argument('--grasp',type=Path,required=True);p.add_argument('--stages',type=Path,required=True);p.add_argument('--reuse-training',type=Path);a=p.parse_args();a.out=a.out.resolve();a.out.mkdir(parents=True,exist_ok=False)
    ...
    if a.reuse_training:
        previous=a.reuse_training.resolve();old=json.loads((previous/'report.json').read_text())
        assert old['protocol_sha256']==report['protocol_sha256']
        development=[r for r in old['runs'] if r['phase']=='development']
        assert len(development)==len(protocol['development'])
        assert all(r['summary']['physical_success'] and r['summary']['protocol_complete'] for r in development)
        data_hash=hashlib.sha256((previous/'dataset.json').read_bytes()).hexdigest()
        old_freeze=json.loads((previous/'final-freeze.json').read_text())
        report['training_reuse']={'source':str(previous),'source_sha':old['source_sha'],
            'report_sha256':hashlib.sha256((previous/'report.json').read_bytes()).hexdigest(),
            'dataset_sha256':data_hash,'scope':'Reuse frozen weights and original development evidence; rerun every final comparison.'}
        report['runs']=[{**r,'reused':True} for r in development]
        shutil.copy2(previous/'dataset.json',a.out/'dataset.json')
        for seed in protocol['training']['seeds']:
            model=previous/('model-'+str(seed));mr=json.loads((model/'report.json').read_text())
            assert mr['complete'] and mr['source_sha']==old['source_sha'] and mr['dataset_sha256']==data_hash
            assert mr['model_sha256']==old_freeze['models'][str(seed)]==hashlib.sha256((model/'act/model.safetensors').read_bytes()).hexdigest()
            shutil.copytree(model,a.out/model.name)
