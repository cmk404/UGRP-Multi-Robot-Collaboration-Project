...
- `rgb-standard-dispatch-v43`: 부모는 v42다. 명시적 `--coordination dynamic`(동기 전용, 새 LLM 계획)에서만 동작이 달라진다. (1) 시작: 세 로봇이 각자 한 번씩 독립적으로 자기 할 일(물체·빔 끝·경로·dock)만 밝히고, 호스트는 서로 맞는지만 확인한다. 맞고 기존 RGB/지도 가능성 검사를 통과하면 협상 없이 출발하고, 충돌하거나 거절되면 기존 동료 협상으로 넘어간다. (2) 실행 중: 빔 짝 접근이 지원된 이유로 실패하면 두 운반 로봇이 자기 RGB·TOP·명령 이력·실패 이유로 retry/abort를 만장일치로 정한다. retry는 짧은 후진(0.05×10구간) 뒤 새 RGB 접근이며 최대 `--approach-retries`(기본 2)다. `--diagnostic-fail-approach-once`는 첫 완료 접근 뒤 실패를 주입하는 진단이다. 기본 `plan_first`의 동작·실패 처리는 v42와 같다.
