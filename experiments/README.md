...
- [2026-09-25 구역 작업 결과 RGB 전용 판정 (L4 대체 후보)](2026-09-25-zone-rgb-outcome/README.md): 감사 L4(정답 기반 작업 영수증·깨우기)를 대체할 독립 모듈 `harness/zone_rgb_outcome.py`(TOP 전후·정적 지도/목표 영역·자기 작업 지정, 선택 자기 RGB). 기록 실행 재렌더로 오프라인 평가(분할 고정, `88b033a` 고정 뒤 test 1회): test 작업 204개 확정 203/204, **잘못된 delivered 0**(모든 시점), 파지 실패 38/38 ≤1 s. 합성 10종 확정 오인 배달 0. 러너 연결은 제안만(교사의 종료 시각 의존 L4b는 남음).
