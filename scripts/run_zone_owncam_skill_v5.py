#!/usr/bin/env python3
"""Runner v5 for ``wrist_zone_skill_v5`` (Codex own-camera review fixes). DIAGNOSTIC unless M1.

A new runner (``zone_owncam_runner_v5``) so the v1-v4 runner stays the recorded
source of its cohorts. Changes against scripts/run_zone_owncam_skill.py:

* ``--mode diagnostic|m1`` (issue 1). ``m1`` refuses to start while the only
  wired pose source is ``gt_stub_eval_only``, and the skill and the outcome
  judge reject it independently. Every result carries the required outcome
  fields of ``harness.m1_contract`` (``m1_success`` separate from
  ``diagnostic_success``, ``counts_as_m1``, ``pose_source``,
  ``input_contract``) and is validated before it is written.
* Coarse order sheet (issue 3): the order gives a 0.5 m pickup bay from the
  order-sheet bay table below; the box position inside the bay and its yaw are
  setup-only (evaluation) values the skill never receives.
* Per-physics-step contact log (issue 5, evaluation only, never fed back):
  cargo-wall, robot-wall and robot-cargo contacts on EVERY physics step, with
  minimum signed distance (negative = penetration) and normal force
  (``mj_contactForce``), plus contact episodes for the wall classes.
* Start-time provenance (issue 6): HEAD, dirty flag and SHA-256 of every
  dependency are recorded before the first physics step and re-checked at the
  end; a change during the run is recorded as ``source_changed_during_run``.
"""
...
RUNNER = 'zone_owncam_runner_v5'
SCHEMA = 'ugrp.zone_owncam_skill.v5'
...
SCENARIOS = {
    ...
    409: {'start': (2.60, -2.60, 0.00), 'bay': 'E1', 'offset': (.10, -.08), 'yaw_deg': 20., 'slot': 'B2'},
    410: {'start': (2.55, 0.80, 0.05), 'bay': 'E3', 'offset': (-.12, .10), 'yaw_deg': -25., 'slot': 'A1'},
    411: {'start': (2.70, -1.20, 0.00), 'bay': 'E2', 'offset': (.05, .12), 'yaw_deg': 12., 'slot': 'C3'},
    # v5 test cohort (pre-registered in experiments/2026-09-25-zone-owncam-skill/README.md)
    541: {'start': (2.55, -2.80, 0.05), 'bay': 'E1', 'offset': (.12, .06), 'yaw_deg': -18., 'slot': 'B3'},
    542: {'start': (2.60, 1.10, 0.00), 'bay': 'E4', 'offset': (-.08, -.10), 'yaw_deg': 27., 'slot': 'A2'},
    543: {'start': (2.65, -1.80, -0.10), 'bay': 'E2', 'offset': (-.10, -.12), 'yaw_deg': -8., 'slot': 'C1'},
    544: {'start': (2.50, 0.20, 0.10), 'bay': 'E3', 'offset': (.14, -.05), 'yaw_deg': 22., 'slot': 'B1'},
    545: {'start': (2.70, -2.30, 0.00), 'bay': 'E1', 'offset': (-.13, .13), 'yaw_deg': 5., 'slot': 'A3'},
    546: {'start': (2.55, 0.60, -0.05), 'bay': 'E4', 'offset': (.06, .12), 'yaw_deg': -28., 'slot': 'C2'},
    547: {'start': (3.00, -1.00, 0.15), 'bay': 'E2', 'offset': (.10, -.02), 'yaw_deg': 16., 'slot': 'B2'},
    548: {'start': (2.60, 0.30, 0.00), 'bay': 'E3', 'offset': (-.05, .14), 'yaw_deg': -22., 'slot': 'A1'},
}
DEV_SEEDS = (409, 410, 411)
TEST_SEEDS = tuple(range(541, 549))
DEPENDENCIES = (
    'scripts/run_zone_owncam_skill_v5.py', 'harness/m1_contract.py', 'harness/wrist_zone_skill.py',
    'harness/wrist_zone_skill_v2.py', 'harness/wrist_zone_skill_v3.py', 'harness/wrist_zone_skill_v4.py',
    'harness/wrist_zone_skill_v5.py', 'harness/visual_box_skill.py', 'harness/visual_box_surface.py',
    'harness/markerless_box.py', 'harness/markerless_face.py', 'harness/approach_geometry.py',
    'harness/visual_arm.py', 'harness/zone_color_boxes.py', 'harness/static_keepouts.py',
...
class PhysicsContactLog:
    ...
    CLASSES = ('cargo_wall', 'r1_wall', 'r1_cargo', 'finger_cargo')
    ...
    def __init__(self, world, box_body):
        ...
        self.episodes = {'cargo_wall': [], 'r1_wall': []}
        self._open = {'cargo_wall': None, 'r1_wall': None}
...
def input_contract(pose_label, static_map):
    ...
    return {'camera': 'robot_cam only; every observation strictly validated (N7 _validate_observation)',
            ...
            'forbidden': ['nav_cam', 'cctv_top*', 'live object pose', 'measured joints', 'contacts/success state'],
            'runner': RUNNER, 'skill_profile': v5.PROFILE}
...
def main():
    ...
    world, definition, config, box_body, order, setup_only = build(args.seed, args.contact_profile)
    ...
    skill = v5.WristZoneDeliveryV5(order, mode=args.mode, planner=make_planner(config['static_map'], order))
    ...
    result = {
        'schema': SCHEMA, 'runner': RUNNER, 'profile': v5.PROFILE, 'seed': args.seed,
