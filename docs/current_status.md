...
- 주 연구 조건 변경(사용자 결정): 통신 없음(`independent`) / 동적 통신(`dynamic`, 분산 한국어 대화) / AI 지휘(`leader`, 중앙 지휘)로 바꾸고 `plan_first`는 은퇴한다(재현용 코드는 유지). `leader` 설계 진행 중, 구현은 #169 뒤. [결정 로그 9/25 조건 변경 절](decision_log.md#2026-09-25--주-연구-조건-변경-무통신동적-통신ai-지휘), [Codex PR 검토](design/2026-09-25-zone-pr-review-codex.md).
