...
기본으로 **STEP 축**을 사용한다. 학습은 원래 optimizer step, 실행은 기록된 결정 순번이다. SIM 시각은 별도의 `execution/sim_time_s` 곡선으로 제공한다. 과거 기록에 실제 시작 시각이 없으므로 **TensorBoard의 WALL/RELATIVE 시간은 변환 시각 기준**이다. 파일 수정 시각에서 실행 시작 시각을 만들어내지 않는다. 연속 영상의 플레이어 시각도 SIM과 자동 정렬하지 않는다.
