...
### 로컬 작업 증거 보관 (사용자 변경 지시, 2026-09-07)

- UGRP는 Google Drive를 사용하지 않는다. 기존 Drive 저장 지시는 철회되었으며 상위 공통 지침보다 이 프로젝트 예외를 우선한다.
- 요청·변경 내용·실행 조건·검증 결과·한계·후속 작업과 원본 증거는 로컬에 보관한다. 코드 버전 관리는 기존 Git 절차를 따른다.
- 별도 요청 없이 Drive 조회·업로드·동기화·업로드 대기 또는 재시도를 하지 않는다. 기존 로컬 자료와 Drive 자료는 삭제하지 않는다.
- 실패 실행도 보존하며, 실제 실행 소스와 이후 수정 소스를 구분한다. 테스트 통과를 실제 운반 성공으로 바꾸지 않는다.
