...
- [2026-09-26 인식 채점 + 접촉 프로필 감사 TensorBoard 스냅샷](2026-09-26-tb-perception-noslip/README.md): PR #193 자기 손목 어안 인식 판단 4종 오프라인 채점(dev·test × belief 3단계)과 PR #189 `cargo_noslip_v1` 부작용 A/B 감사 42 실행을 새 스냅샷 `0926-zone-perception-noslip` 89 run으로 변환했다. 인식은 판단별 정확도·unknown 비율·확신 오류와 사전 등록 게이트 8개(전부 통과, 결정 56뷰 오답 0, unknown 25–46%, σ60 mm에서 붕괴)를, 감사는 프로필을 HParam으로 둔 시나리오별 A/B와 게이트 324/327 통과, 기준 프로필의 화물 낙하(hold 89.45 s 접촉 0 → 89.5 s 바닥)를 원본 `trace.jsonl` 표본 곡선으로 보여준다. 기록 대조 315건·이벤트 재독 스칼라 8,731개·시계열 530,910점 불일치 0, 공용 서버(PID 9291/9293) 재시작 없이 89 run 표시 확인. 첫 내보내기 6건 실패(구역 문자 대문자 태그)와 원본 README 수치 2건 불일치도 기록했다. 새 실험·SIM·모델 호출은 없고 두 집합을 합산하지 않는다.
