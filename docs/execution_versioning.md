...
- `rgb-standard-dispatch-v48`: 부모는 main의 v47이다. 기본 OFF인 `--navigation planned`를 추가한다. 동기식 skills 실행기에서 세 로봇 모델이 상자의 목적지(dock)와 하역 뒤 대기 위치(`park`: 지도 region 이름 또는 `xy_m` 좌표)를 정하고, 상자 로봇의 적재 경로와 대기 이동 경로는 정적 지도 위 A*(`harness/map_goto.py`)가 현재 TOP RGB 추정 위치에서 계획한다. 저작 장애물·지형·RGB 장벽·빔 팀 금지 영역을 피하며, 실행 전 가능성 검사가 경로/대기 위치를 거절하면 이유를 모델에 돌려 재합의시킨다. 빔은 합의한 north/south 통로를 그대로 쓰고, 이 모드는 직렬 자원만 지원한다(`--route-overlap` 거절). 정답 위치·접촉·성공 신호를 쓰지 않는다. 기본 authored 경로와 기록은 바뀌지 않으며 v43 이전 결과를 승계하지 않는다.
