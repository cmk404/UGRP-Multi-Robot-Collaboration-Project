...
class MasterPiPhysicsWorld:
    ...
    def _center_visible_target(self, max_iters: int = 5, det: dict | None = None) -> tuple[bool, dict]:
        # Reuse a fresh caller-supplied observation. The old implementation
        # rendered the exact same settled pose twice at the start of most
        # visual-servo iterations.
        det = self.camera_red_detection() if det is None else det
        if not det.get("visible"):
            return False, det
        for _ in range(max_iters):
            err = 0.5 - float(det["cx"])
    ...
    def act(self, action: str) -> ActionResult:
        ...
        if action == "search":
            ...
            centered, det = self._center_visible_target(det=det)
        ...
        if action == "approach":
            ...
            for _ in range(approach_iters):
                ...
                if float(det.get("cy", 0.0)) >= 0.66:
                    centered, det = self._center_visible_target(max_iters=8, det=det)
                    if not centered:
                ...
                centered, det = self._center_visible_target(max_iters=4, det=det)
                if not centered:
                    return ActionResult(False, action, "lost the red target during visual approach", self.state())
        ...
        if action == "pick":
            ...
            for _ in range(sample_count):
                sample = self.camera_red_detection()
            ...
            for i, c in enumerate(np.linspace(0.0, 1.0, close_samples)):
                ...
                if i % max(1, close_samples // 12) == 0 or i == close_samples - 1:
                    self._emit_frame()
            if os.environ.get("UGRP_PICK_REQUIRE_BILATERAL", "0") == "1":
        ...
        if action == "place_on_yellow":
            ...
            gentle = os.environ.get("UGRP_GENTLE_PLACE_TRANSPORT", "0") == "1"
            if gentle:
                self.move_base_relative(yaw_delta=yaw_delta, frames=14, max_w=0.42, a_ang=1.35)
                self.close_gripper(1.0)
                if not self.contact_state().bilateral:
            ...
            if forward > 1e-6:
                # One smooth profile, with lower acceleration in the guarded
                # variant to preserve a marginal friction grasp.
                if gentle:
