...
class PairNavigationScene(ShortTransportScene):
    ...
    def __init__(self, out, grasp_root, data, *, impratio=1, noslip_iterations=0, spacing_integral=0., finger_friction_damping=0):
    ...
    def open(self):
        ...
        def builder(*args, **kwargs):
            ...
            configure_finger_contacts(root,self.finger_friction_damping)
...
def run(data, grasp_root, out, budget=750, *, impratio=1, vision_mode='legacy', grasp_spacing=None, grasp_only=False, close_pulse=None, noslip_iterations=0, spacing_integral=0., finger_friction_damping=0):
    validate_map(data)
    ...
    scene = PairNavigationScene(out, grasp_root, data, impratio=impratio, noslip_iterations=noslip_iterations,spacing_integral=spacing_integral,finger_friction_damping=finger_friction_damping)
    ...
    report = {'schema': 'ugrp.pair_navigation_trial.v1', 'map': data, 'map_sha256': digest(data),
              ...
              'finger_friction_damping':finger_friction_damping,'spacing_integral':spacing_integral,'grasp_spacing':grasp_spacing,'grasp_only':bool(grasp_only),
...
def main():
    ...
    parser.add_argument('--close-pulse',type=int,choices=(1500,1600,1700,1800),help='visual spacing defaults to 1600; passive retains the skill command; no contact feedback')
    ...
    result = run(json.loads(args.map.read_text()), args.grasp_model_dir.resolve(), args.out_dir.resolve(), args.budget,
        impratio=args.impratio, vision_mode=args.vision_mode, grasp_spacing=args.grasp_spacing, grasp_only=args.grasp_only,close_pulse=args.close_pulse,noslip_iterations=args.noslip_iterations,spacing_integral=args.spacing_integral,finger_friction_damping=args.finger_friction_damping)
