...
- [2026-09-26 구역 대화 연구 no-LLM 오프라인 스모크](2026-09-26-zone-study-offline-smoke/README.md): 연구 패키지 A(계약)·C(프로토콜)·D(SIM 비용)·E(시나리오)·I(평가)를 한 브랜치로 통합하고 저장 손목 RGB 참조 + 가짜 시계로 6 시나리오 × (주 4조건 + 참조 상한 R) × seed 1 = 30회를 돌렸다(`d87e0f2`, 모델 호출 0회·물리 0회). 30/30에서 채널 격리(`no_comm` 송수신 0, `leader_ko` follower끼리 0, `structured` 자유 문장 0, `peer_ko` 한국어 18/18)·비용 회계(호출 비용 = `released−requested`, 합계 = 스케줄러 사고 시간)·평가 역류 없음(비공개 구역만 바꾼 재실행에서 SIM trace·로그·입력 해시 동일 30/30)을 확인했다. 지휘자는 r1/r2/r3를 모두 덮는다. **프로토콜 배선만 검증했고 언어 이해·통신 효과·물리 운반 성공은 입증하지 않는다.** 물리가 없어 어떤 시행도 성공으로 집계되지 않는다(전 조건 0/6, censored 6/6).
  ...
  - [review-r5](2026-09-26-zone-study-offline-smoke/review-r5/README.md): Codex 5차 검토 3건을 고쳤다(`f44ea845`). 고친 항목은 사용량 미상 예외가 이미 보낸 재시도의 예산을 돌려주던 문제(P1), summary 전용 기록의 미상 표지 소실(P2), 같은 seed 반복 시행의 TensorBoard run 병합(P2, run 이름 `<condition>/<trial_id>`)이다. 회귀 테스트 61개는 수정 전 코드에서 61/61 실패하고, 변이 21개 중 20개가 이 테스트에서 걸렸다(1개는 동치). 메모리 내 두 시행이 v4와 같아 스모크를 재실행하지 않았다(v5 없음). v4 스냅샷은 그대로 둔다. 모델 호출 0회·물리 0회.
  - [review-r6](2026-09-26-zone-study-offline-smoke/review-r6/README.md) · [v5](2026-09-26-zone-study-offline-smoke/v5/README.md): Codex 6차 검토 2건과 통합 PR #229가 찾은 2건을 고쳤다(`8ab8f7ed`). 고친 항목은 네 가지다. `submit()`이 전송 뒤 예외를 내면 예산을 돌려주고 기록에서 빠지던 문제(P1: `NotSent`만 반환, 보낸 수는 `sent_attempts`), 심볼릭 링크로 logdir 밖에 쓰던 문제(P2: 실제 경로 검사), 재질문 타이머 누적(로봇당 대기 1개), 계약 v2(tags_v2 지도 키, v1 보존)다. 회귀 테스트 85개는 수정 전 코드에서 82개가 실패하고(3개는 대조군), 변이 38개가 모두 이 테스트에서 걸렸다. 재질문 규칙이 채널 3조건의 SIM trace를 바꿔 v5로 재실행했다. 30/30 통과했고, 채널 조건 호출은 시행당 90회에서 72–75회로 줄었으며 종료가 `budget_exhausted`에서 `sim_horizon`으로 바뀌었다. `no_comm`·`reference_R` 12회는 v4와 SIM trace가 같다. TensorBoard 스냅샷 v5(run 35개)를 새로 만들었다. 모델 호출 0회·물리 0회. 호출 수 변화는 규칙 수정의 결과이며 통신 효과가 아니다.
