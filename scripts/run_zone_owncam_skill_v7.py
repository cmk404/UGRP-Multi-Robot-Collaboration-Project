#!/usr/bin/env python3
"""Runner v7 for ``wrist_zone_skill_v7`` (re-plan to another box face after a keep-out guard stop). DIAGNOSTIC.

A new runner (``zone_owncam_runner_v7``) so runner v6 stays the recorded source of
its cohort (551-560). Changes against scripts/run_zone_owncam_skill_v6.py:

* skill ``wrist_zone_skill_v7``;
* peers parked RELATIVE TO THE BOX (``parked``: r2 and optionally r3), so the
  face in view / the default west grasp position is blocked in most scenarios
  (v6 552/557 failure mode). Their parking spots are static-layout keep-outs
  (``StaticKeepout`` 'static_map_parking'); the controller never reads live
  peer poses;
* the path planner adds the delivery's own-RGB box footprint
  (``planner_discs``) while re-planning instead of the whole coarse bay.

Diagnostic like v5/v6: GT pose stub, so ``m1_success`` is always False.
"""
...
RUNNER = 'zone_owncam_runner_v7'
SCHEMA = 'ugrp.zone_owncam_skill.v7'
...
SCENARIOS = {
    ...
    415: {'start': (2.60, -1.50, 0.00), 'bay': 'E2', 'offset': (.05, -.06), 'yaw_deg': 0., 'slot': 'B2',
          'parked': {'r2': (-.25, .30)}},
    416: {'start': (2.55, 0.40, 0.05), 'bay': 'E3', 'offset': (-.08, .06), 'yaw_deg': -20., 'slot': 'A1',
          'parked': {'r2': (-.30, -.28)}},
    417: {'start': (2.60, -2.40, 0.00), 'bay': 'E1', 'offset': (.10, .05), 'yaw_deg': 0., 'slot': 'C3',
    ...
    561: {'start': (2.55, -2.30, 0.00), 'bay': 'E1', 'offset': (.08, .05), 'yaw_deg': 0., 'slot': 'B1',
          'parked': {'r2': (-.25, .30)}},
    562: {'start': (2.60, -1.20, 0.05), 'bay': 'E2', 'offset': (-.10, .08), 'yaw_deg': 0., 'slot': 'A2',
          'parked': {'r2': (-.28, -.30)}},
    563: {'start': (2.50, 0.30, 0.00), 'bay': 'E3', 'offset': (.06, -.10), 'yaw_deg': 2., 'slot': 'C1',
          'parked': {'r2': (-.30, .25)}},
    564: {'start': (2.60, 1.00, -0.05), 'bay': 'E4', 'offset': (-.05, -.05), 'yaw_deg': -2., 'slot': 'B3',
          'parked': {'r2': (-.25, -.30)}},
    565: {'start': (2.70, -2.60, 0.10), 'bay': 'E1', 'offset': (-.12, .10), 'yaw_deg': 0., 'slot': 'A3',
          'parked': {'r2': (-.28, .28), 'r3': (0., -.40)}},
    566: {'start': (2.55, -1.70, 0.00), 'bay': 'E2', 'offset': (.12, -.04), 'yaw_deg': 15., 'slot': 'C2',
          'parked': {'r2': (-.30, .30)}},
    567: {'start': (2.60, 0.20, 0.00), 'bay': 'E3', 'offset': (-.06, .12), 'yaw_deg': -25., 'slot': 'B2',
          'parked': {'r2': (-.25, -.32)}},
    568: {'start': (2.65, 0.80, 0.00), 'bay': 'E4', 'offset': (.10, -.08), 'yaw_deg': 0., 'slot': 'A1'},
    569: {'start': (2.60, -1.40, 0.00), 'bay': 'E2', 'offset': (.04, .00), 'yaw_deg': 0., 'slot': 'C3',
          'parked': {'r2': (0., .40)}},
    570: {'start': (2.55, -2.10, 0.15), 'bay': 'E1', 'offset': (.00, .06), 'yaw_deg': 0., 'slot': 'B1',
          'parked': {'r2': (-.30, 0.)}},
    # the two v6 guard-stop layouts (v6 552 / 557, parking spot converted to box-relative)
    571: {'start': (2.60, -1.20, 0.05), 'bay': 'E2', 'offset': (-.10, .08), 'yaw_deg': 0., 'slot': 'A2',
          'parked': {'r2': (-.40, .22)}},
    572: {'start': (2.60, 0.20, 0.00), 'bay': 'E3', 'offset': (-.06, .12), 'yaw_deg': -30., 'slot': 'B2',
          'parked': {'r2': (-.44, -.12)}},
}
DEV_SEEDS = (415, 416, 417, 418, 419)
TEST_SEEDS = tuple(range(561, 573))
PARKING_IDS = {'r2': 'parking_P1', 'r3': 'parking_P2'}
DEPENDENCIES = (
    'scripts/run_zone_owncam_skill_v7.py', 'scripts/run_zone_owncam_skill_v6.py', 'scripts/run_zone_owncam_skill_v5.py', 'harness/m1_contract.py', 'harness/wrist_zone_skill.py',
    'harness/wrist_zone_skill_v2.py', 'harness/wrist_zone_skill_v3.py', 'harness/wrist_zone_skill_v4.py',
    'harness/wrist_zone_skill_v5.py', 'harness/wrist_zone_skill_v6.py', 'harness/wrist_zone_skill_v7.py', 'harness/visual_box_skill.py', 'harness/visual_box_surface.py',
...
def input_contract(pose_label, static_map):
    ...
    return {'camera': 'robot_cam only; every observation strictly validated (N7 _validate_observation)',
            ...
            'runner': RUNNER, 'skill_profile': v7.PROFILE}
...
def main():
    ...
    holder = {}
    skill = v7.WristZoneDeliveryV7(order, mode=args.mode, static_keepouts=keepouts,
    ...
    result = {
        'schema': SCHEMA, 'runner': RUNNER, 'profile': v7.PROFILE, 'seed': args.seed,
