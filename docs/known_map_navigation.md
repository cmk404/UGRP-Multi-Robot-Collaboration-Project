...
기본 `--motion-style heading`은 길을 향해 차체를 돌린 뒤 전진한다. 초기 전진 영상 변위로 앞 방향을 구분하고, 공용 영상에 나타난 차체 외형의 회전으로 방향 변화를 확인한다. 보정 중에도 옆걸음 명령을 쓰지 않는다. 카메라의 장착 위치나 FOV는 바꾸지 않으며, 차체가 회전할 때 부착 카메라도 물리적으로 함께 움직인다. 이전의 몸 방향 고정·전진/옆걸음 혼합 방식은 `--motion-style holonomic`으로 남겨 비교할 수 있다. 이것은 접은 팔 상태의 차체 주행이며, 별도의 목이나 카메라 pan 동작을 추가한 것은 아니다.

지도 범위는 계획 영역이며 그 선에 물리 벽을 추가하지 않는다. 보정은 0.6초 동작과 영상 정지 확인을 축별 최대 6회 반복하며, 누적 영상 변위 5cm를 요구한다. heading은 전진 축만, holonomic은 전진과 옆걸음 축을 보정한다. 각 동작의 여유 공간 검사는 현재 시뮬레이터와 시험한 시작 조건에서 검증한 것으로, 임의의 실물 구동기에 대한 최대 이동 거리 보장은 아니다. `direct`도 같은 지도 기반 초기 보정 검사를 통과하므로 비교 결과는 주행 중 경로계획의 효과를 보여준다.
...
검증 설계와 전체 결과는 [기존 지도 주행](../experiments/2026-09-13-known-map-navigation/)과 [회전 후 전진 비교](../experiments/2026-09-13-heading-map-navigation/)를 따른다. 기존 코호트 실행기는 재현을 위해 holonomic을 기본값으로 유지한다. 두 동작 방식을 같은 시작 조건에서 비교하려면 `scripts.run_known_map_cohort --matrix heading-comparison --max-steps 400`을 사용한다.
