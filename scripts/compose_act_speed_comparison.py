...
def load_video(folder,cache):
    ...
    if abs(times[0]-report['evaluation_initial_state']['sim_time_s'])>.02:raise ValueError('initial timestamp mismatch')
    if abs(times[-1]-report['final_physics']['sim_time_s'])>.6:raise ValueError('final timestamp mismatch')
    record={'path':str(p.resolve()),'sha256':digest(p),'frames':len(frames),'timestamps':times,'ocr':ocr,
            'success':report['success'],'approach_sim_s':report['approach_elapsed_sim_s'],
...
def main():
    ...
    p.add_argument('--cases',nargs='+',default=['in_domain-01','distance_shift-07','lateral_shift-05','yaw_shift-05'])
    a=p.parse_args();out=a.out.resolve();out.mkdir(parents=True,exist_ok=True)
    specs=[('기존 ACT','slow_act','final-baselines-v1'),('2배 명령 교사','teacher2','final-baselines-v1'),
           ('빠른 ACT · 학습 1','fast_act16','final-fast16-v1'),('빠른 ACT · 학습 2','fast_act17','final-fast17-v1')]
    ...
    cv2.setNumThreads(1)
    for case in a.cases:
        sources=[load_video(ROOT/'outputs'/root/(case+'-'+condition),out/(case+'-'+condition+'-timestamps.json')) for _,condition,root in specs]
        first=sources[0][1][0]
        ...
        try:
            for n in range(nframes):
                ...
                draw.text((22,1690),'전진·정지만 학습; 파지·들기는 공통 스킬. 실패 시 접근 종료 시간은 성공 완료 시간이 아닙니다.',font=small,fill='#b4bfcc')
