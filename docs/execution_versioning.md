...
- `rgb-standard-dispatch-v57`: 부모는 main의 v54다. PR #144(v54)가 먼저 병합되어, R2 재생이 쓴 v55를 바이트 그대로 은퇴 보존하고 같은 `--navigation planned` 동작을 v54 위에 다시 등록했다(v56은 다른 브랜치가 사용 중). `--plan-guidance objective*`와의 결합은 거절한다. v55·v48 후보의 결과를 승계하지 않는다.
