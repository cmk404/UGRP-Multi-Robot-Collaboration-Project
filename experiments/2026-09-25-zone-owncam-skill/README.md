...
## v7: guard 정지 뒤 다른 면으로 재계획 (2026-09-26, v6 552·557 후속)
**변경 (`harness/wrist_zone_skill_v7.py`, 러너 `scripts/run_zone_owncam_skill_v7.py`)**
- v1–v6은 바이트 그대로다. v6은 M1(#201)이 채택하는 인터페이스라 해시로 고정했다(v7 테스트).
- **파지 중 `STATIC_KEEPOUT_GUARD`가 나면 끝내지 않고 재계획한다.**
  1. 상자 지도 위치: N7 바닥 적합 중심 + 자기 pose 추정. 면 yaw: 마지막 top-edge 투표 + 자기 heading. 둘 다 자기 RGB에서 나온다.
  2. 지금 보던 면(로봇 쪽)은 막힘으로 표시한다.
  3. 나머지 면 k마다 접근점(상자 + 법선 × 0.45 m)과 파지 base 위치(× 0.165 m, v6 551·556·559 측정 0.162–0.167)를 잇는 직선을 검사한다. 모든 정적 원판에서 로봇 반경 + 원판 반경 + 0.05 m 떨어져야 하고, 정적 경계 안이어야 한다.
  4. 통과하는 면이 없으면 **`NO_GRASPABLE_FACE_CLEAR_OF_KEEPOUTS`로 깨끗하게 중단**한다.
  5. 있으면 원판 쪽으로 가지 않는 후퇴(≤ 0.12 m) → 가장 가까운 면 접근점 주행 → 새 박스 스킬로 다시 파지한다. 경로 계획은 bay 전체 대신 자기 RGB 상자 원판을 쓴다. 재계획은 최대 2회, 넘으면 `KEEPOUT_REPLAN_LIMIT`.
- bay 접근 후보에 **동쪽**(서쪽을 바라봄)을 마지막 순서로 추가했다(v7에서만).
- 러너 v7은 주차 로봇을 **상자 기준** 위치에 둔다(r2, 필요하면 r3). 주차 자리는 정적 keep-out(`static_map_parking`)으로만 스킬에 전달된다.
- 평가 전용 면 오차는 delivery가 `grasp` 단계인 프레임만 센다. 418 dev의 `replan_nav` 중에는 이전 박스 스킬의 쓰지 않는 옛 정렬 값이 기록되기 때문이다. v6 수치(159/159)는 이 정의로 다시 계산해도 같다.

**개발 실행** (`outputs/.../dev-v7/`, 합산 금지)

| 실행 | 배치 | 결과 |
|---|---|---|
| 415-a (dirty) | r2 상자 북서 | 남쪽 bay 접근, 재계획 0, IN_SLOT ✓ 149.9 s |
| 418-a (dirty) | v6 552 배치(E3) | 서쪽 측면 접근 → guard → **재계획 1회(남쪽 면)** → IN_SLOT ✓ 231.4 s. 파지 단계 면 오차 ≤ 1.38°, 상대 접촉 0 |
| 416 (`e026dd5`) | r2 상자 남서, yaw −20° | 북쪽 접근, IN_SLOT ✓ 147.3 s |
| 419 (`e026dd5`) | r2 상자 남서, 북쪽은 경계 밖 | **동쪽** 접근, IN_SLOT ✓ 168.4 s |
| 417 (실행 안 함) | r2·r3로 서·남, 북은 경계 | 사전 계산에서 bay 단계 `BAY_APPROACH_BLOCKED`(깨끗한 중단 경로). 면 단계 중단은 단위 테스트로 확인 |

## v7 사전 등록 (시험 고정 커밋 전에 작성)
- 시드 **561–572**(12개). 모두 bay E1–E4.
  - 11개는 상자 옆에 로봇이 주차해 있다(10개 r2, 565는 r2·r3).
  - 568은 주차 없는 대조군이다.
  - **571·572는 v6 552·557의 배치**를 상자 기준으로 옮긴 것이다.
  - 축 정렬(yaw 0·±2)이 9개, 회전(+15, −25, −30)이 3개다.
- 조건 **P**: 러너 v7 `--mode diagnostic --contact-profile cargo_noslip_v1`, 시드당 1회, GT stub(**M1 아님**). SIM 420 s, 900단계.
- 판정 기준
  - K1 `diagnostic_success` ≥ 10/12, 그리고 **571·572 둘 다 성공**(v6 실패 배치).
  - K2 **r1–상대 로봇 접촉 0 step**(12/12). 거짓 IN_SLOT 0. 스킬 판정과 GT 일치 ≥ 11/12.
  - K3 실패는 모두 깨끗한 중단이어야 한다: `NO_GRASPABLE_FACE_CLEAR_OF_KEEPOUTS`, `BAY_APPROACH_BLOCKED`, `KEEPOUT_REPLAN_LIMIT` 중 하나이고 IN_SLOT 주장이 없다. **최종 사유가 `STATIC_KEEPOUT_GUARD`인 실행 0**(재계획 없이 끝난 guard 정지 금지).
  - K4 면 법선은 모두 top-edge 투표에서 나온다. 파지 단계 준비 프레임의 ≥ 95%가 5° 이내다(평가 전용).
  - K5 계약: `validate_outcome`, `counts_as_m1 = false`, `cameras_seen = [robot_cam]`, 관측 거부 0, 소스 변경 없음, 깨끗한 시작. weld 0.
  - K6 등록 시드 12개가 모두 있다.
  - 보고 항목: 재계획 수와 선택 면, bay 접근 방향, SIM 시간, 단계 수.
- 중단 규칙
  - 시험 커밋 고정. 거짓 IN_SLOT이 나오면 멈춘다. 첫 제어 단계 전 인프라 예외만 1회 재실행한다.
  - 동시 SIM 2개(이 코호트가 두 레인을 다 쓴다), 스레드 1. 코호트 뒤 조정은 새 버전으로만 한다.

## v7 결과 (사전 등록 시드 561–572, 소스 `57e2fd1`, 진단 모드 — **M1 아님**)
**인프라 예외.** 566–572는 첫 실행에서 모두 거부됐다. 첫 제어 단계 전에 러너가 작업 트리가 깨끗하지 않다며 막았다(제 실수로 v8 파일을 코호트 worktree에 만들었다). 사전 등록한 예외에 따라, 같은 SHA의 분리된 동결 worktree(`ugrp-wt/zone-owncam-skill-v7-frozen`)에서 1회 재실행했다. 거부 로그는 `<seed>.refused-dirty-tree.log`로 보존했다. 564·565는 그때 이미 실행 중이었다. 끝날 때 `dirty_source_end = true`가 기록됐지만, v7은 추적되지 않은 v8 파일을 불러오지 않는다. 그래서 `source_changed_during_run`은 비어 있다(의존 파일 해시 동일).

| 기준 | 결과 | 판정 |
|---|---|---|
| K1 성공 ≥ 10/12, 571·572 성공 | **11/12**, 571 ✓ 200.4 s, 572 ✓ 247.0 s(v6 552·557 배치) | 통과 |
| K2 상대 접촉 0, 거짓 IN_SLOT 0, 판정 일치 ≥ 11/12 | 접촉 0 step(12/12), 거짓 0, 일치 12/12 | 통과 |
| K3 실패는 모두 등록된 깨끗한 중단, guard 최종 정지 0 | guard 최종 정지 0. 단 **570이 `GRASP_TARGET_NOT_VISIBLE`**(등록 목록 밖, IN_SLOT 주장 없음) | **실패** |
| K4 파지 단계 면 ≥ 95%가 5° 이내 | 190/190 | 통과 |
| K5 계약 | `validate_outcome` 12/12, `counts_as_m1 = false`, `robot_cam`만 사용, 관측 거부 0, 소스 변경 0, 깨끗한 시작 12/12, weld off. 564·565 종료 시 dirty 표기는 위 설명 참고 | 통과(주석) |
| K6 시드 12개 | 12/12 | 통과 |

- 재계획: 565·570·571·572에서 1회씩 일어났다. 570을 뺀 3개가 성공했다.
- 접근 방향은 south 5, north 3, west 3, east 1이다. SIM 116–252 s, 239–504단계.
- **570 원인(평가 전용 진단).** guard가 파지 후반에 나서, 팔이 내려간 자세(자기 명령 3:500 / 4:2384) 그대로 새 박스 스킬이 시작됐다. 손목 카메라가 정면 0.44 m(GT)의 상자를 보지 못했다. 418·565는 파지 초반(3:633)에 guard가 나서 성공했다. 수정은 v8에서 한다(v7은 고정).
- 결론: **v7은 K3 때문에 사전 등록 전체 통과가 아니다.** 재계획 자체(guard 정지 뒤 다른 면 파지)는 4번 중 3번 성공했고, 상대 접촉은 0이었다.
- TensorBoard `0926-zone-owncam-skill-v7`(26 run: v7P 12, v6P 기준선 10, v7dev 4). view 키 `zone_owncam_skill_v7_20260926`.

## v8: 접근 단계 청록 채도 기준을 프레임 상대값으로 (2026-09-26, M1 dev-a6 차단 요인)
**문제.** M1 dev-a6(v6 사용)의 s93(축 정렬)·s95(−20°)가 서쪽 PICKUP 바닥에서 `GRASP_TARGET_NOT_VISIBLE`로 실패했다.
- 원인: N7 접근 단계 `observe_ground_box(min_saturation=65)`. 서쪽 바닥은 밝은 파랑(H≈103, S≈94–118)이라 청록 범위 안에 든다.
- s93은 접근 프레임을 모두 놓쳤다. s95 371 프레임은 바닥을 상자로 보고 (0.98, −0.32) m를 냈다. v6 top-edge yaw 마스크(S ≥ 70)도 같은 문제가 있다.

**변경 (`harness/wrist_zone_skill_v8.py`, 러너 `scripts/run_zone_owncam_skill_v8.py`)**
- v1–v7은 바이트 그대로다. v6·v7 동결본은 테스트에서 해시로 고정했다.
- `WristZoneDeliveryV8`은 v6/v7과 같은 공개 API의 drop-in이다.
- 조명 정책은 Kiro B의 PR #193 `harness/zone_own_perception.py` @ `fb44c2c`를 따랐다(코드는 새로 작성, 출처 표기). 색 기준은 프레임과 주변 바닥·페인트에 상대적으로 잡고, 절대 하한은 회색 거부에만 쓴다.
- **프레임마다 바닥 자기 색에서 기준을 정한다.**
  - 유효 시야 아래 70%에서 바닥 색을 추정한다. 40% 이상이 S ≥ 30이면 색 바닥으로 본다.
  - 색 바닥: 중앙 hue ± 8 군집의 S 90번째 백분위수. 회색 바닥: 영역 S의 90번째 백분위수.
  - `min_saturation = clip(max(65, 바닥 S_p90 + 35), 65, 150)`.
- 같은 기준을 top-edge yaw에도 쓴다. 기준 미만 픽셀을 회색으로 바꾼 뒤 v6 추정기를 돌린다(검정이 아니어서 v6 가장자리 잘림 검사를 건드리지 않는다).
- 패치 범위: 두 패치 모두 해당 `decide`/`observe` 호출 안에서만 유효하다(테스트로 확인).
- 러너 v8: 서쪽 PICKUP bay W1–W4를 추가했고 문 통과를 포함한다. 동쪽 E bay는 회귀 확인용이다.
- **두 번째 수정: 재계획 뒤 팔 자세 (v7 코호트 570, 평가 전용 진단).**
  - 570은 파지 후반에 guard가 나서 팔이 내려간 자세(자기 명령 3:500 / 4:2384)였다.
  - v7은 그 자세 그대로 새 박스 스킬을 시작했다. 손목 카메라가 집게 밑 바닥만 봐서 정면 0.44 m(GT)의 상자를 20프레임 동안 보지 못했다(`GRASP_TARGET_NOT_VISIBLE`).
  - 418·565는 guard가 파지 초반(3:633)에 나서 성공했었다.
  - v8은 후퇴가 끝나면 재계획 주행 전에 N7 SEARCH 자세를 명령한다. 도착 시 자기 마지막 명령이 여전히 SEARCH가 아니면 한 번 더 명령한 뒤 새 박스 스킬을 시작한다. 판단 근거는 로봇 자신의 발행 명령뿐이다.
  - `replan_nav` 중 기록된 상자 "visible" 값은 옛 박스 스킬이 남긴 기록이다. 제어에는 쓰이지 않았다.

**재생 검증** (평가 전용, `tests/fixtures/wrist_zone_skill_v8/`에 M1 프레임 5장·SHA 보존)

| 프레임 | 65 (v6) | v8 기준 | 150 |
|---|---|---|---|
| 동쪽 v5–v7 954장 | 945 검출 | 938 검출, GT 오차 같음(기준 65–104) | — |
| M1 s93 | 7 검출(3개 비현실적 위치) | 125 검출, 비현실적 0 | — |
| M1 s95 | 7 검출(6개 비현실적) | 43 검출 | 44 |

- 동쪽 프레임의 top-edge yaw p90 오차 1.35°(v6와 동급).
- s95에서 5° 이내 36/41. 비교 기준이 로봇 자기 heading 추정이라 거친 확인이다.

**개발 실행** (`outputs/.../dev-v8/`, 합산 금지)

| 실행 (`6bdacb2`, 깨끗한 트리) | 배치 | 결과 | 바닥 기준 | 접근 상자 검출(평가 전용) | 면 오차(파지 단계) |
|---|---|---|---|---|---|
| 420 | W2, yaw 0 | 서쪽 접근, IN_SLOT ✓ 194.1 s | 148 | 63/71, 거짓 위치 0, 최대 2.6 cm | 8/8 ≤ 5° |
| 421 | W1, −20°(M1 s95 각) | 서쪽 접근, IN_SLOT ✓ 197.8 s | 150 | 81/81, 거짓 0, 최대 1.9 cm | 24/27 ≤ 5°(최대 7.7°) |
| 422 | W3, +2° | 서쪽 접근, IN_SLOT ✓ 159.8 s | 150 | 65/69, 거짓 0, 최대 2.5 cm | 10/10 |
| 423 | v7 570 배치(E1) | 남쪽 → guard → **SEARCH 복귀 1회** → 재계획 1회 → IN_SLOT ✓ 216.2 s | 82 | 94/94, 거짓 0 | 19/19 |

- 모든 dev 실행에서 상대 접촉 0, 화물–벽 접촉 0, 문 통과 포함.
- dev는 합산하지 않는다. 시험 판단은 아래 사전 등록 코호트로만 한다.

## v8 사전 등록 (시험 고정 커밋 전에 작성)
- 시드 **581–591**(11개).
  - **서쪽 PICKUP 바닥 8개**(581–588, W1–W4 각 2개). 축 정렬(yaw 0·±2)이 6개(581–584, 587, 588), 회전은 585(−20°, M1 s95와 같은 각), 586(+15°).
  - 587은 r2가 상자 옆에 주차해 있다(서쪽 바닥의 v7 재계획 경로).
  - 동쪽 회귀 2개: 589(E1, yaw 0), 590(E3, −25°).
  - **591**: v7 570 배치의 변형(E1, r2가 접근 쪽에 주차). 재계획 뒤 SEARCH 자세 복귀를 확인한다.
- 조건 **P**: 러너 v8 `--mode diagnostic --contact-profile cargo_noslip_v1`(**사용자 결정 대기 프로필**), 시드당 1회, GT stub(**M1 아님**). SIM 420 s, 900단계. weld OFF.
- 판정 기준
  - W1 서쪽 `diagnostic_success` ≥ 7/8, 그리고 축 정렬 서쪽 6개 중 ≥ 5.
  - W2 **`GRASP_TARGET_NOT_VISIBLE`로 끝나는 실행 0**(11/11).
  - W3 거짓 IN_SLOT 0, 상자 검출 거짓 위치 0. 평가 전용 기준은 접근 단계에서 검출 중심이 GT 상자 중심에서 0.25 m 넘게 벗어난 프레임이 없어야 한다. 스킬 판정과 GT 일치 ≥ 10/11.
  - W4 동쪽 589·590·591 모두 성공. 591에서 재계획이 일어나면 새 박스 스킬 시작 시점의 자기 발행 자세가 SEARCH여야 한다.
  - W5 로봇–상대 접촉 0 step. 실패는 모두 깨끗한 중단(IN_SLOT 주장 없음)이어야 한다.
  - W6 계약: `validate_outcome`, `counts_as_m1 = false`, `cameras_seen = [robot_cam]`, 관측 거부 0, 소스 변경 없음, 깨끗한 시작. 등록 시드 11개가 모두 있다.
  - 보고 항목: 프레임별 바닥 기준(`last_floor_gate`), 면 오차, 문 통과, SIM 시간, 단계 수.
- 중단 규칙
  - 시험 커밋 고정. 거짓 IN_SLOT이 나오면 멈춘다. 첫 제어 단계 전 인프라 예외만 1회 재실행한다.
  - 동시 SIM 2개, 스레드 1. 코호트 중에는 이 worktree에 파일을 만들지 않는다(v7 사고 재발 방지: 코호트는 분리한 동결 worktree에서 실행). 코호트 뒤 조정은 새 버전으로만 한다.

## v8 결과 (사전 등록 시드 581–591, 소스 `39f214c`, 분리된 동결 worktree, 깨끗한 트리, 진단 모드 — **M1 아님**)

| 기준 | 결과 | 판정 |
|---|---|---|
| W1 서쪽 ≥ 7/8, 축 정렬 서쪽 ≥ 5/6 | 서쪽 **6/8**, 축 정렬 서쪽 **4/6** | **실패** |
| W2 `GRASP_TARGET_NOT_VISIBLE` 0 | **587** 1건 | **실패** |
| W3 거짓 IN_SLOT 0, 접근 상자 거짓 위치 0, 판정 일치 ≥ 10/11 | 거짓 0, 거짓 위치 0(최대 3.2 cm), 일치 11/11 | 통과 |
| W4 동쪽 589·590·591 성공 | 3/3. 591은 재계획이 일어나지 않아 SEARCH 복귀는 **시험되지 않음**(dev 423만 근거) | 통과(범위 주의) |
| W5 상대 접촉 0, 실패는 깨끗한 중단 | 접촉 0, 두 실패 모두 IN_SLOT 주장 없음 | 통과 |
| W6 계약, 시드 11개 | `validate_outcome` 11/11, `robot_cam`만 사용, 깨끗한 시작·끝, weld off, 누락 0 | 통과 |

- 성공 9/11(서쪽 6/8, 동쪽 3/3). SIM 111–205 s. 실패는 583 `SIM_LIMIT` 420 s, 587 `GRASP_TARGET_NOT_VISIBLE` 68 s.
- 바닥 기준: 서쪽 132–150(8개 중 7개가 상한 150), 동쪽 74–87.
- 접근 단계 상자 검출은 GT에서 0.25 m 넘게 벗어난 프레임이 0이다(최대 3.2 cm). v6 기준 65에서 나던 서쪽 오검출(M1 s95 유형)은 이 코호트에 없다.
- 면 오차(평가 전용): 748/767이 5° 이내. 서쪽 584·586·588에서 최대 10–15°가 나왔고, 세 시드 모두 성공했다.
- **583 원인(평가 전용 진단).** 상자 위치는 GT와 6 mm 이내로 정확했다. 그런데 손목(3번) 545 ↔ 561에 따라 N7 적합이 윗면 ↔ 상자 전체로 번갈아 바뀌었다. `pixel_centroid` 정의가 달라져 세로 오차가 −31 ↔ +39 px로 뒤집혔다. 면 접근 중에는 v2의 강제 전진이 꺼져 있어 0.29 m 앞에서 약 600단계 동안 손목만 흔들렸다.
- **587 원인(평가 전용 진단).** 파지 단계가 상자 0.27 m 앞에서 시작됐다(bay 안 상자 위치 −0.10 m가 접근 쪽으로 치우침). 기준 140–150에서는 옅은 옆면(S p10 93)이 빠져 상자 전체 적합의 IoU가 약 0.5에 그쳤다. 더 낮은 기준에서는 바닥 조각과 겹쳐 후보가 여러 개가 됐다. 손목을 올려 둘러보는 사이 상자가 화면 아래로 잘렸다.
- 결론: **v8은 사전 등록 전체 통과가 아니다(W1·W2).** 서쪽 바닥 오검출 해소는 확인됐다(거짓 위치 0). 남은 두 실패는 v9 범위로 넘긴다(코디네이터 지시: 두 수정만).

## v9: v8 583·587의 두 실패만 고침 (2026-09-26, 코디네이터 지시, 범위 제한)
**변경 (`harness/wrist_zone_skill_v9.py`, 러너 `scripts/run_zone_owncam_skill_v9.py`)**
- v1–v8은 바이트 그대로다(v6·v7·v8 동결본은 테스트에서 해시로 고정). `WristZoneDeliveryV9`는 v6–v8과 같은 API의 drop-in이다.
- 판단 근거는 자기 RGB 적합과 자기 발행 명령뿐이다.
- **(a) 583 — 적합 모델에 따라 뒤집히는 세로 오차**
  - 세로 오차를 N7 `pixel_centroid` 대신 **추정 3D 상자 중심을 영상에 투영한 점**으로 계산한다. 윗면·상자 전체 어느 적합에서 나와도 같은 점이다.
  - 583 재생: v8은 −31 / +39 px로 뒤집혀 허용 범위(±25)를 양쪽으로 넘었다. v9는 −4 / +20 px로 둘 다 범위 안이다.
  - 면 접근 중 진척 guard: 적합이 유효한데 몸체 이동 없이 손목 보정만 6번 연속이면, 상자가 정면(bearing ≤ 0.10 rad)이고 0.20 m보다 멀 때 한 번 전진한다(v2 강제 전진과 같은 0.08×0.3).
- **(b) 587 — 근거리에서 적합을 잃음 (내려다보는 자세로 재적합)**
  - 진단 정정: 파지 단계는 약 0.41 m에서 시작했고 적합도 정상이었다. N7의 0.15 m 전진 한 번으로 0.27–0.30 m에 들어간 뒤, 남쪽 접근에서는 기본 팔꿈치(4번 2320)로 적합이 성립하지 않았다.
    - 서쪽 바닥에서 상자 앞면 아래쪽이 바닥 반사로 옅어져(S p10 93) 실루엣이 잘렸다(IoU 약 0.5).
    - 밝은 윗면 사각형이 앞면 위쪽 띠와 합쳐졌다.
  - 첫 dev(`1561166`, 후진만): 424(= 587 배치)가 다시 실패했다. 적합은 약 0.36 m에서 돌아오고 0.34 m에서 다시 사라졌다.
  - 진단 스윕(개발 전용, 424 배치에 로봇을 남쪽 거리별로 설치, `robot_cam` 프레임 적합; GT는 평가만): 팔꿈치 2480 + 손목 몇 값 중 하나로 0.34–0.18 m 모든 거리(0.32 m 제외)에서 1 cm 이내 적합이 나왔다.
  - 조치: 근거리 전진 직후 적합을 잃으면 **내려다보기 모드**로 들어간다.
    - 팔꿈치 2480, 손목 560→530→500→620→680을 한 프레임씩 시도한다.
    - 스윕이 모두 실패하면 짧은 후진(v1/v2 retreat) 뒤 다시 스윕한다. 후진은 상자당 최대 6걸음이고, 그 뒤 마지막 유효 자세로 한 번 복귀하고 N7에 맡긴다.
    - 모드 중에는 전진을 짧은 전진(0.10×0.6)으로 제한한다. 투영 중심이 목표 행 ±80 px 안이면 세로 보정을 멈춰 자세를 유지한다.
- 러너 v9: v8과 같은 장면·bay·keep-out·제한. dev 424(= v8 587), 425(= v8 583), 426(W4 −15°).

**재생·단위 검증**
- fixture: `tests/fixtures/wrist_zone_skill_v9/`, v8 코호트 583 step 296·297, 587 step 88·89, SHA 보존.
- 테스트 9개: 583 부호 뒤집힘 제거, 진척 guard, 내려다보기 진입·스윕·후진·복귀·N7 반환, 세로 유지, 원거리 미발동, API 유지, 동결본 해시.

**개발 실행** (`outputs/.../dev-v9/`, 합산 금지)

| 실행 | 배치 | 결과 | v9 동작 | 접근 상자 검출(평가 전용) |
|---|---|---|---|---|
| 424 `1561166`(후진만) | v8 587 | `GRASP_TARGET_NOT_VISIBLE` 63 s | 후진 2회, 0.34 m에서 다시 잃음 | 13/51, 거짓 위치 0 |
| 425 `1561166` | v8 583 | IN_SLOT ✓ 184 s | 투영 중심 98프레임 | 98/106, 거짓 0 |
| 426 `1561166` | W4 −15° | IN_SLOT ✓ 192 s | 진척 guard 1회 | 106/106, 거짓 0 |
| **424 `77773c5`** | v8 587 | **IN_SLOT ✓ 218 s** | 내려다보기 진입 1, 스윕 19프레임, 세로 유지 10 | 38/70, 거짓 0(최대 1.2 cm) |
| **425 `77773c5`** | v8 583 | **IN_SLOT ✓ 170 s** | 내려다보기 1, 스윕 5, 진척 guard 1 | 65/75, 거짓 0 |
| **426 `77773c5`** | W4 −15° | **IN_SLOT ✓ 192 s** | 진척 guard 1(`1561166`과 동일 궤적) | 106/106, 거짓 0 |

- 모든 dev 실행에서 상대 접촉 0, 화물–벽 접촉 0.

## v9 사전 등록 (시험 고정 커밋 전에 작성)
- 시드 **601–610**(10개). 591은 v8에서 이미 써서 601부터 시작한다(코디네이터 예시 591–600에서 변경).
  - **서쪽 PICKUP 바닥 8개**(601–608, W1–W4 각 2개).
    - 축 정렬(yaw 0·±2) 6개: 601, 602, 603, 604, 605, 608.
    - 회전: 606(−20°), 607(+15°).
    - 605는 587과 같은 형태로 r2가 서쪽에 주차해 남쪽에서 접근한다. 608도 r2가 주차해 있다(북쪽 접근).
  - 동쪽 회귀 2개: 609(E1 yaw 0), 610(E3 +20°).
- 조건 **P**: 러너 v9 `--mode diagnostic --contact-profile cargo_noslip_v1`(**사용자 결정 대기 프로필**), 시드당 1회, GT stub(**M1 아님**). SIM 420 s, 900단계. weld OFF.
- 판정 기준
  - V1 서쪽 `diagnostic_success` ≥ 7/8, 그리고 축 정렬 서쪽 6개 중 ≥ 5.
  - V2 **`SIM_LIMIT` 0, `GRASP_TARGET_NOT_VISIBLE` 0**(583·587 유형 재발 없음).
  - V3 거짓 IN_SLOT 0, 접근 단계 상자 거짓 위치 0(GT 중심에서 0.25 m 초과, 평가 전용), 스킬 판정과 GT 일치 ≥ 9/10.
  - V4 동쪽 609·610 둘 다 성공.
  - V5 상대 접촉 0 step. 실패는 모두 깨끗한 중단(IN_SLOT 주장 없음)이어야 한다.
  - V6 계약: `validate_outcome`, `counts_as_m1 = false`, `cameras_seen = [robot_cam]`, 관측 거부 0, 소스 변경 없음, 깨끗한 시작, 10개 시드 모두 있음.
  - 보고 항목: `v9_stats`(투영 중심, 진척 guard, 내려다보기 진입·스윕·후진·복귀·짧은 전진·세로 유지), 바닥 기준, 면 오차, SIM 시간, 단계 수.
- 중단 규칙
  - 시험 커밋 고정. 코호트는 분리한 동결 worktree에서 실행한다. 거짓 IN_SLOT이 나오면 멈춘다. 첫 제어 단계 전 인프라 예외만 1회 재실행한다.
  - 동시 SIM 2개, 스레드 1. 코호트 뒤 조정은 새 버전으로만 한다.

## v9 결과 (사전 등록 시드 601–610, 소스 `04a5e3c`, 분리된 동결 worktree, 깨끗한 트리, 진단 모드 — **M1 아님**)

| 기준 | 결과 | 판정 |
|---|---|---|
| V1 서쪽 ≥ 7/8, 축 정렬 서쪽 ≥ 5/6 | 서쪽 **8/8**, 축 정렬 서쪽 **6/6** | 통과 |
| V2 `SIM_LIMIT` 0, `GRASP_TARGET_NOT_VISIBLE` 0 | 0, 0 | 통과 |
| V3 거짓 IN_SLOT 0, 접근 상자 거짓 위치 0, 판정 일치 ≥ 9/10 | 거짓 0, 거짓 위치 0(최대 3.7 cm), 일치 10/10 | 통과 |
| V4 동쪽 609·610 | 2/2 | 통과 |
| V5 상대 접촉 0, 깨끗한 중단 | 접촉 0, 실패 없음 | 통과 |
| V6 계약, 시드 10개 | `validate_outcome` 10/10, `robot_cam`만 사용, 깨끗한 시작·끝, 소스 변경 0, weld off, 누락 0 | 통과 |

- **성공 10/10**. SIM 157–234 s, 325–460단계. 화물–벽·로봇–벽·상대 접촉 모두 0.
- 접근 방향: 서쪽 8, 남쪽 1(605, 587형), 북쪽 1(608). 바닥 기준은 서쪽 143–150, 동쪽 73–86.
- v9 동작(자기 입력 기반)
  - 내려다보기 진입 4회: 602, 603, 605, 606. 스윕 3–26프레임, 후진 0, 복귀 0.
  - 면 접근 진척 guard 3회: 601, 602, 610.
  - 투영 중심 세로 오차는 모든 적합 프레임에 적용됐다.
- 587형 605(남쪽 접근, r2 주차)는 내려다보기 스윕 26프레임으로 적합을 다시 잡고 234 s에 성공했다. 접근 적합 35/74, 거짓 위치 0.
- 면 오차(평가 전용): 137/148이 5° 이내.
- 범위: 진단 모드(GT 자세 stub)라 M1 성공이 아니다. 서쪽 바닥의 587형 남쪽 접근은 표본 1개(605)와 dev 424뿐이다.
- TensorBoard `0926-zone-owncam-skill-v9`(v9P 10, v8P 기준선 11, v9dev 6). view 키 `zone_owncam_skill_v9_20260926`.
