...
def run(data,grasp_root,out,mode,duration_s=300, *, contact_profile='retention', slip_guard=True, recovery=True):
    ...
    skill,gm=models(grasp_root,'student-skill.json');scene=EnduranceScene(out,grasp_root,data,impratio=10,contact_profile=contact_profile)
    actors={r:EnduranceActor(data,r,mode,slip_guard=slip_guard) for r in ROBOTS};sync=PairCarrySync(data['map_id']+'-endurance')
    report={'schema':'ugrp.pair_grasp_endurance.v1','source_sha':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
        ...
        'contact_profile':contact_profile,'slip_guard':slip_guard,'recovery_enabled':recovery,'recoveries':[],
    ...
    try:
        ...
        for i in range(round(duration_s/DT)):
            ...
            if not all(d['ready'] for d in decisions.values()):
                if recovery and slip_requested(decisions):
                    retry=len(report['recoveries'])==0
                    rec=recover_pair(scene,gm,len(report['recoveries'])+1,regrasp=retry)
                    rec['trigger_index']=i;report['recoveries'].append(rec)
                    ...
                    continue
                raise RuntimeError('RGB endurance stopped: '+str({r:d.get('error') for r,d in decisions.items()}))
...
def main():
    ...
    p.add_argument('--duration',type=int,choices=(30,60,300),default=300)
    p.add_argument('--contact-profile',choices=('baseline','retention'),default='retention')
    p.add_argument('--no-slip-guard',action='store_true');p.add_argument('--no-recovery',action='store_true')
    a=p.parse_args();r=run(json.loads(a.map.read_text()),a.grasp_model_dir.resolve(),a.out_dir.resolve(),a.mode,a.duration,
        contact_profile=a.contact_profile,slip_guard=not a.no_slip_guard,recovery=not a.no_recovery)
