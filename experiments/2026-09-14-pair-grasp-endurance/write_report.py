...
import json
base=Path('experiments/2026-09-14-pair-grasp-endurance');data=json.loads((base/'records/summary.json').read_text());groups=data['groups'];final=next(x for x in groups if x['label']=='final-baseline');r=final['trials'];points=json.loads((base/'media/plot-data.json').read_text());total=sum(len(x['trials']) for x in groups)
rows=[]
for t in r:
 e=t['evaluation'];c=e['checkpoints'];rows.append(f"| {t['case']} | {c['180']['last_sample_s']:.2f}초 | 실패 | 실패 | {t['decision_rounds']} | {t['wall_seconds']:.1f}초 |")
text=f'''# 공동 파지 장시간 검증: 현재 3분·5분 모두 실패

현재 파지 설정은 정지 유지와 왕복 운반 모두 약 94초에 물체가 미끄러져 크게 기울어지고, own RGB 실루엣 guard로 공동 정지한다. 180초·300초 유지 성공은 없다. 최종 실행 SHA `{final['source_sha']}`. 파지/주행/물리 변경 후보는 모두 제외했고, 최종 변경은 장시간 시험 도구와 출력 전용 평가·감사·기록이다.
...
stationary는 RGB 위치 유지, shuttle은 12초 주기·목표 0–16cm 왕복이다. 원본 평가에서 중심 x 이동 범위는 각각 {points[0]['actual_center_x_span_m']*100:.2f}cm / {points[1]['actual_center_x_span_m']*100:.2f}cm다. 외부 LLM 호출 0, 비용 0이다. 초기 grasp 학생 추론과 영상 기하·결정론적 제어를 사용하며 LLM 협업 성능을 뜻하지 않는다.
...
각 후보는 stationary/shuttle 두 설정으로 실행했으나 모두 endurance 진입 전 같은 초기 구간에서 실패했다. 총 물리 실행 {total}회이며 성공한 변경 후보는 없다. 실패를 평가에서 제외하거나 본래 성공으로 바꾸지 않는다. 상세 설정·실행 SHA는 [사전 프로토콜](protocol.md), 명령·결과·경고는 [전체 기록](records/summary.json)에 있다.
...
- 저장된 RGB·명령·공동 허가의 정확한 재생 감사가 모든 {total}회에 통과했다. 입력 감사 통과는 물리 성공과 구분한다. 제외된 후보를 감사하려면 해당 호환 과거 소스가 필요하며, 최종 감사는 제외된 설정을 거부한다.
