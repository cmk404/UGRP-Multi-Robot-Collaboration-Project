...
def evaluate_endurance(rows, report):
    ...
    return {'success':bool(checks) and all(x['passed'] for x in checks.values()),'checkpoints':checks,'grasp_stability':grasp}
...
def run(data,grasp_root,out,mode,duration_s=300):
    ...
    report={'schema':'ugrp.pair_grasp_endurance.v1','source_sha':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),
        'map':data,'map_sha256':digest(data),'mode':mode,'duration_s':duration_s,'close_pulse':1600,'impratio':10,'noslip_iterations':0,
