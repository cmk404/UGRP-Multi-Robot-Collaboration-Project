...
- `rgb-standard-dispatch-v54`: 부모는 main의 v53이다. PR #146 등이 먼저 병합되어, PR #144 코호트가 실행한 v45를 바이트 그대로 은퇴 보존하고 같은 `--plan-guidance` 동작을 v53 위에 다시 등록했다. `--plan-guidance`는 `--coordination plan_first`에서만 허용한다(dynamic 자기 주장 경로는 별도 프롬프트이며 검증하지 않음). v45 코호트 결과를 승계하지 않는다.
