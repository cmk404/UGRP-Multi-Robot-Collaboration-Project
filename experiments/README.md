...
- [2026-09-26 구역 교사 막힘 수정](2026-09-26-zone-teacher-fix/README.md): PR #169 A2 막힘 B2·B3·B4·B5·B8을 하위 클래스·게이트·선택 인식 프로필로 다뤘다(PR #169 파일·`CONDITIONS`·`ModeLoop` 불변). **교사 실현 가능성 전용**(정답 교사, fixture, LLM 0회, weld OFF, `cargo_noslip_v1` 사용자 승인 대기). 새 seed 21–23 코호트 5/8 달성. 실패 3회는 모두 seed 22의 새 막힘 N1(시작 TOP에서 green 상자 채도 미달로 미검출, open)이다. B3·B4는 seed 12 재실행으로 수정 확인, B5는 오프라인 확인, B2·B8은 물리 실행 전 게이트 거부(선언 순서 문제는 open).
