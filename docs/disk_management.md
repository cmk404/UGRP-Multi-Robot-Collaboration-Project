...
| 구분 | 측정 (09-26 15:26) | 이번 적용 뒤 | 제안 예산 |
|---|---|---|---|
| 기본 체크아웃 `outputs/` | 66.74 | 67.56 (Codex 은퇴분 0.82 유입) | ≤ 40 |
| worktree (69개) | 78.07 | 63.82 (56개, 15:49) | ≤ 10 |
| 기본 체크아웃 나머지(`experiments/` 1.01, `.git` 0.96, `work/` 1.13 등) | 3.47 | 3.47 | ≤ 6 |
| 가상환경(`Project-Runtimes/ugrp`) | 1.12 | 1.12 | ≤ 2 |
| **프로젝트 합계** | **149.4** | **약 136** | **≤ 60** |
| 에이전트 상태(`~/.codex` worktree 제외 5.68, `~/.kiro` 0.67, `~/.claude` 0.19) | 6.55 | 6.55 | 예산 밖, 보고만 |

- 이번 적용은 병합된 Codex-app worktree 14개 은퇴다. checkout 14.38 GiB(추정)를 확보했다. 파일시스템 여유는 32.47 → 46.88 GiB로 늘었는데, 이 값에는 다른 작업의 쓰기가 섞여 있다.
...
AGENTS.md에 따라 raw 삭제·압축·이동은 사용자가 정한다. 아래 크기는 09-26 15:26 측정이며 폴더 이름 기반 분류라 추정치다. `python3 scripts/disk_report.py --sections outputs,retention`이 최신 목록과 기록 참조 여부를 보여 준다.

| 등급 | 대상 | GiB | 제안 |
|---|---|---|---|
| T0 기반 자료 | `tensorboard*`, `agent-locks`, 모델 설치본 | 0.46 | 유지 |
| T1 test 코호트 raw | 이름에 test·final·holdout·cohort가 있는 폴더 | 8.53 | **그대로 유지**(주장 근거) |
| T2 기록이 커밋된 dev·진단 raw | 이름에 dev·diag·pilot·probe·smoke·sweep이 있는 폴더 | 11.81 | 솎기 또는 외부 보관 |
