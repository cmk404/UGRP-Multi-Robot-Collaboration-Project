...
- [2026-09-26 시뮬레이션 속도 (동작 불변)](2026-09-26-sim-speed/README.md): 비트 동일 `--speedups exact-v1`로 M1 instruction −16.6–18.3%(s93·s95 전체 임무 명령·프레임 바이트·`result.json` 동일), pair −4.5%. 과부하 시 같은 구간 CPU 2.3배 → `scripts/sim_slots.py` 대기열, DEV 전용 앞부분 slice. Kiro PR #209.
