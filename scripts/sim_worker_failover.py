...
LOCAL='ugrp-sim-worker.service'
RECOVERY='ugrp-sim-gpu-recover.service'
POLL=1.0
RECOVERY_MISS_LIMIT=3   # 3 seconds before asking a configured GPU provider to recover.
RECOVERY_COOLDOWN=60.0  # Retry transient allocation/start failures once per minute.
PROVIDER_PROBE_INTERVAL=15.0
RECOVERY_ACTIVITY_MAX_AGE=75.0  # Never wake a paid/managed GPU without recent SIM use.
...
def configured_provider_names() -> set[str]:
    try:
        # Reload the private provider environment on every probe. This means a
        # user can add/rotate .env.gpu while the watchdog is already running;
        # no daemon restart is needed before the next probe.
        cmd = (
            "set -a; "
            "[ ! -f .env.gpu ] || . ./.env.gpu; "
...
def main():
    ...
    # Production policy is GPU-only. The Oracle host is orchestration only.
    # Even if a stale/manual local worker happens to be running, remove it now.
    if local_active():
        print('GPU-only policy: stopping Oracle CPU MuJoCo worker', flush=True)
        systemctl('stop', LOCAL)
    ...
    while True:
        ...
        # Never allow a local MuJoCo worker to remain active in production.
        if local_active():
            print('GPU-only policy: removing unexpected Oracle CPU worker', flush=True)
            systemctl('stop', LOCAL)
        ...
        if remote and not (provider_allowed and contract_allowed):
            if authoritative and inflight is None:
                if set_remote_authority(False):
                    authoritative=False
                    print(f'rejected incompatible GPU worker provider={remote_provider!r} contract={health.get("remote_worker_contract")!r}; allowed_providers={sorted(configured_providers)} expected_contract={REMOTE_WORKER_CONTRACT}', flush=True)
        ...
        if remote:
            ...
            if not authoritative and inflight is None:
                ...
                if instance_id and instance_id != last_instance_id:
                    print(f'new GPU worker {instance_id}: episode boundary handled in bridge/executive', flush=True)
                if set_remote_authority(True):
                    last_instance_id=instance_id or last_instance_id
                    print('GPU worker authoritative', flush=True)
                else:
        ...
        if should_start_recovery(
            ...
            print('GPU worker absent: requesting configured GPU-provider recovery', flush=True)
            result=systemctl('start','--no-block',RECOVERY)
            last_recovery_attempt=now
            if result.returncode != 0:
                print(f'GPU recovery service start failed: {result.stderr.strip()}', flush=True)
        time.sleep(POLL)
