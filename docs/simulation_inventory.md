...
2026-09-23 관리 통합: 아래는 최초 연결 당시의 검토 기록이다. 현재 실행 진입점과 기록 방식은 [표준 시뮬레이션 관리](simulation_management.md)를 따른다. 19개 workflow를 실제 실행·기록·조회할 수 있게 연결했고, 출하장 실행기의 XML/초기화는 표준 `Scene`을 공유한다. 다른 호환 실행기의 제어 루프까지 재작성한 변경은 아니다.
