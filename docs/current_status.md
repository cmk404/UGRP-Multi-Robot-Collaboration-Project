...
2026-09-25 사용자 요청에 따라 연구는 SIM 시간의 로봇 행동에 집중한다. 실시간 모드와 wall 시간·시뮬레이션 속도 개선은 다루지 않는다. 아래는 모두 조건마다 1회라 사례로만 읽는다.
...
[실시간 출하 실행](realtime_dispatch.md)은 `dispatch --realtime-control`로 물리·관측·판단·MuJoCo 창을 분리하는 선택 경로다. 현재 저장 계획 open/seed11 조건에서 진단하며, 후보별 성공·실패와 고정 실행 소스는 [실험 기록](../experiments/2026-09-23-realtime-dispatch/README.md)에 둔다. 실시간 시계 복구, 명령 연속성, 화물 도착·하역은 각각 검증한다. 이 경로는 main에 있다. 2026-09-25 이후로는 실시간 경로를 연구 대상으로 삼지 않는다.
