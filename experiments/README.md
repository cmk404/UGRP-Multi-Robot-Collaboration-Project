...
- [2026-09-26 자기 손목 카메라 인식 v3.1: 정보 없는 영상은 `unknown` (PR #193 검토 P1)](2026-09-26-zone-own-perception-v3-1/README.md): v1·v2·v3를 바이트 그대로 두고 v3를 영상 정보 게이트(밝은 비율·밝기 폭·엣지·선명도)로 감쌌다. 검정·V 0–39·균일 회색·흐림·렌즈 덮개 영상은 모든 판단·자세에서 `unknown`, 모양 답은 경계가 보일 때만, 부재는 밝은 띠에서만. v3 test는 재채점하지 않고 새 test 371–374와 조명 3종 그림자 stress 381–382를 사전 등록 뒤 1회 채점. 밝고 매끈한 부분 가림막은 열린 한계. 제어 미연결.
- [2026-09-26 자기 손목 카메라 인식 v3: 집게 위치 예측 영역·운반 중 팀 화물·파지 단계 관측 자세 (패키지 B v3)](2026-09-26-zone-own-perception-v3/README.md): v1·v2를 바이트 그대로 두고 발행 PWM의 순기구학으로 “내 집게의 물건이 보여야 할 자리”를 예측해 그 안의 색만 센다(v2 W7 수정). W7의 원인은 청록 상자가 아니라 pickup 바닥 칠(H 98–100)이었고, v2의 “검은 운반 crate”는 렌즈가 몸체 안에 들어가는 fixture 인공물이었다. 파지 단계 관측 자세를 5개 후보에서 골라 `tri_frame` 종류와 손잡이를 한 프레임에서 묻는다. dev/test 각 4시드·100뷰, 라벨은 평가 전용, 사전 등록 게이트 뒤 test 1회 채점. 제어 미연결.
