...
기준: 2026-09-25, main `80df7f4`에 포함된 기록(아래 9/25 절). 그 아래 절은 적힌 날짜의 기록이다. 9/26 절은 **미병합 draft PR의 상태**이며 main의 결과가 아니다. 이 문서는 진입점이며 실험 결과는 연결된 보고서의 실행 SHA·조건에만 적용된다. 새 결과를 병합하면 이 문서와 README의 요약을 함께 갱신한다.
...
- 병합됨(PR #177, 2026-09-26 확인): 자기 카메라 위치 추정 1단계. 벽 AprilTag 지도 버전 `*_tags_v1`(기존 지도 불변), wrist 어안 PnP, 발행 명령 파티클 필터. 교사 주행 오프라인 평가에서 둘러보기 자세의 test 문 근처 오차는 p50 1.9 cm / p90 5.8 cm다(사후 분석, 사전 등록 아님). 사전 등록 게이트는 G2(전체 p50)만 통과하고 G1·G3은 실패했으며, 주 원인은 상자를 든 수평 carry의 태그 가시율 0%다. 사후 진단에서 20° 숙인 carry는 가시율 97%였다. 2단계 폐루프 문 통과는 PR #178에서 진행 중이다. [기록](../experiments/2026-09-25-zone-owncam-loc/README.md), [문서](zone_owncam_localization.md).
...
- 주 연구 조건 변경(사용자 결정): `plan_first`는 은퇴한다(재현용 코드 유지). 같은 날 후속으로 통신 채널만 바꾸는 축을 제안했다. 조건은 무통신 / 자유 한국어 대화 / 한국어 명령 지휘자 / 정형 메시지 통제이며, 전지적 지휘자는 참고용 상한이다. 로봇 LLM 입력에서 TOP 카메라를 빼고(평가 전용), 정적 지도와 시나리오 설정의 작업 지시서를 준다. 실시간 상태는 주지 않는다. 지휘자는 로봇 한 대가 겸하고 시행마다 r1/r2/r3로 돌린다(허브-스포크, 사용자 승인). 대화·사고 시간의 SIM 비용 모델은 2026-09-26 draft PR #186에서 구현했고 러너에는 아직 연결하지 않았다(아래 9/26 절). [결정 로그 9/25 조건 변경 절](decision_log.md#2026-09-25--주-연구-조건-변경-무통신동적-통신ai-지휘), [Codex PR 검토](design/2026-09-25-zone-pr-review-codex.md).
...
## 대화 연구 착수와 자기 카메라 진행 — 2026-09-26 (모두 미병합)

기준 `origin/main` `1cd9ea1`. 이 절의 PR은 전부 **draft·미병합**이며, 병합은 사용자 확인·승인 뒤에만 한다. 세부는 [결정 로그 9/26절](decision_log.md#2026-09-26--kiro-서브에이전트-추가-대화-연구-작업-패키지-착수-자기-카메라-진행과-미끄러짐-판정)을 따른다.

- **에이전트 구성:** 사용자 결정으로 **Kiro가 Claude·Codex에 이어 서브에이전트로 추가됐다.** `claude-opus-5`·effort `max`(`~/.kiro/settings/cli.json`)로 돌고, 브랜치 접두사는 `kiro/`이며 여러 세션을 Claude와 병렬로 쓴다. `AGENTS.md` 규칙 등록은 다른 세션의 PR #182가 소유한다.
- **연구 작업 패키지(Codex 통합 설계 9절):** Kiro draft PR로 A 계약·입력 #187, C 프롬프트·통신 #184, D SIM 비용·scheduler #186, E 시나리오 설정 #190, I 평가·지표 #185가 올라갔다. B 자기 RGB 인식은 아직 로컬 작업 중이고 PR이 없다. 부수 작업은 noslip 부작용 감사 #189, 보고서 초안 #191, 한국어 파일럿 후속 수정 #188, TensorBoard 보완 #183이다. 러너 통합(G)·실행 메뉴(H)·로봇 실행(F)은 미착수다.
- **검증 범위:** 위 PR이 통과시킨 것은 계약·비용·지표 계층의 단위 테스트다. **4조건으로 실제 실행한 결과는 아직 없고**, 통신 효과의 근거로 쓸 수 없다. #186 비용 기본값은 전부 잠정이고 러너에 연결되지 않았다. #189의 A/B 42건도 실행 전이다.
- **wrist 스킬 격리 결과:** 사전 등록 시드에서 v1 1/5(501–505), v2 9/10(511–520), v3 주 조건 P `cargo_noslip_v1` 9/10(521–530, 보조 S `local_contact_fine` 10/10). **모두 `pose_source=gt_stub_eval_only`이고 문 통과가 없어 M1이 아니다.** 공용 자세는 `CARRY_POSTURE`·`LOOK_P20`(`harness/owncam_drive.py:35-36`)로 합의했다. [기록](../experiments/2026-09-25-zone-owncam-skill/README.md).
- **단독 운반 미끄러짐 판정(PR #181):** `local_contact_fine`의 약 2 mm/min creep은 물리가 아니라 soft 마찰 제약의 감쇠-only 기준에서 오는 **시뮬레이터 수치 인공물**이다(30 g 상자에 법선력 합 10.4 N, `cargo_noslip_v1`에서는 약 0.016 mm/min).
- **보류 중인 사용자 결정:** `cargo_noslip_v1`을 **연구 전체 접촉 프로필로 채택할지는 아직 정하지 않았다.** `noslip_iterations`는 전역 solver 옵션이라 장면의 모든 접촉에 영향을 주며, 채택 여부는 #189 부작용 감사 결과를 보고 정한다. 기본값은 바뀌지 않았다.
