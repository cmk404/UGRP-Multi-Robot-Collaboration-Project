...
def assess(report_path):
    ...
    for saved in report['runs']:
        ...
        commands=json.loads((root/'issued-commands.json').read_text()) if (root/'issued-commands.json').exists() else None
        decisions=json.loads((root/'pair-decisions.json').read_text()) if (root/'pair-decisions.json').exists() else []
        ...
        evidence.append({'trial_id':saved['trial_id'],'condition':saved['condition'],
            ...
            'commands':sum(map(len,commands.values())) if isinstance(commands,dict) else None,
    ...
    for name,count in counts['conditions'].items():
        ...
        qualified=(counts['complete'] and count['attempted']==count['planned'] and count['failures']==0
                   and matching and all(t['required_evidence_complete'] and t['report_matches_readback']
