#!/bin/zsh
# v5 pre-registered cohort: arm P 541-548, runner v5, diagnostic mode (gt_stub pose, NOT M1),
...
OUT=/Users/changmin/projects/ugrp/outputs/zone-owncam-skill-20260925/cohort-v5-$SHA
mkdir -p $OUT
LOG=$OUT/cohort.log
...
run1() {
  ...
  python3 scripts/run_zone_owncam_skill_v5.py --mode diagnostic --seed $seed --contact-profile cargo_noslip_v1 "$@" \
...
echo "$(date -u +%FT%TZ) cohort v5 source=$SHA dirty=$(git status --porcelain | wc -l | tr -d ' ')" >> $LOG
( for s in 541 543 545 547; do run1 P $s; done ) &
( for s in 542 544 546 548; do run1 P $s; done ) &
wait
