...
[2026-09-16 ACT 확대 학습](../experiments/2026-09-16-act-feasibility/README.md)은 고정 사전학습 영상 인코더, 정지 구간 균형 샘플링, 개발 체크포인트 선택과 새 거리쌍20개를 검증한다. 같은50개 학습 시연에서 커널20/20, ACT 두 초기값 각각20/20 접근·파지 성공이다. ACT는 접근·정지만 학습하며 파지는 공통 스킬을 사용한다. 기존 커널의 성능/비용을 넘었다는 결과는 아니므로 기본값은 유지한다.
