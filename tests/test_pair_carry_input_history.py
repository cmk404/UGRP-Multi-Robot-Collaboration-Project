...
@pytest.mark.parametrize('stop_mode,arrived,distance',[
    ('learned',False,12),('rgb_guarded',True,0),('rgb_guarded',False,12),
    ('rgb_refined',True,0),('rgb_refined',False,12),('rgb_refined',False,81)])
def test_runtime_preserves_robot_windows_and_executed_pause_context(tmp_path,monkeypatch,stop_mode,arrived,distance):
    import json
    ...
    monkeypatch.setattr(arrival,'arrival_evidence',lambda *args:{'arrived':arrived,'error_px':[0.,distance]})
    import harness.dispatch_skill_binding as binding
    monkeypatch.setattr(binding,'ImageRoute',lambda *args:SimpleNamespace(
        observe=lambda rgb:None,points=[1,2,3],index=0,confirmations=9))
    class Pair:
        calls=[]
        now=0.
        ...
        def drive_mecanum(self,actions,dt):
            ...
            self.now+=dt
        def carry(self,navigator,max_steps):
            assert navigator.index==2 and navigator.confirmations==0 and max_steps==10
            self.refined=True
    pair=Pair();pair.calls=[]
    if not arrived and (stop_mode=='rgb_guarded' or stop_mode=='rgb_refined' and distance>80):
    ...
    assert len(admissions)==(0 if stop_mode=='learned' else 4 if stop_mode=='rgb_guarded' and arrived else 1)
    assert bool(getattr(pair,'refined',False))==(stop_mode=='rgb_refined' and not arrived and distance<=80)
    if stop_mode!='learned':assert admissions[0]['requesting_slots']==['r1']
    pair.calls=[r for r in pair.calls if r['kind']=='act_carry']
    count=1 if stop_mode=='rgb_refined' or stop_mode=='rgb_guarded' and not arrived else 5
    assert Client.closed and len(pair.calls)==count
    assert pair.calls[0]['inputs']['r1']['history']==[pair.calls[0]['inputs']['r1']['history'][0]]*4
    assert pair.calls[-1]['ready_count']==(0 if count==1 else 3)
    assert pair.tracked==[b'top']*(count+1)  # Anchor plus every frame, including done.
