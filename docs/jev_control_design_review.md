...
현 단계 권고는 **RGB에서 얻은 의미 있는 상태와 실행 가능한 짧은 동작 후보를 Jev에 주고, 영상 피드백 제어기와 역할을 나누는 것**이다. 직접 이동 선택을 포기할 근거는 없지만, 일곱 방향 버튼을 매번 고르는 현재 구성을 최적이라고 볼 근거도 없다. 아래 제안은 후속 검증 후보이며 아직 개선된 자율 주행 성공이 아니다.
...
| 단계 | 비교 | 알 수 있는 것 |
|---|---|---|
| 1: 표현만 개선한 폐루프 | 기존 7동작 그대로, 원래 숫자 입력 vs 의미 상태; Jev와 Gemini 모두 제공, 규칙 유지 | 이번 72회 판단 개선이 실제 접근 성공으로 이어지는지 |
...
1단계에는 기존 3자세를 회귀로 남기고 개발에 쓰지 않은 시작 위치/방위 조합 12개를 별도 고정한다. 같은 관측기에서 가능한 범위를 먼저 검증한다. 조건당 3회, 같은 씨앗과 상태를 비교군에 짝지으며 실행 순서를 섞는다. 별도 개발 조건에서만 문구·기간·임계값을 조정한다. 변경된 Jev 하네스를 Gemini에도 적용한 비교와 모델별 최적화 비교를 구분한다.
