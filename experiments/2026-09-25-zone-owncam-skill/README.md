...
- 판정 기준
  ...
  - W2 **`GRASP_TARGET_NOT_VISIBLE`로 끝나는 실행 0**(10/10).
  - W3 거짓 IN_SLOT 0, 상자 검출 거짓 위치 0. 평가 전용 기준은 접근 단계에서 검출 중심이 GT 상자 중심에서 0.25 m 넘게 벗어난 프레임이 없어야 한다. 스킬 판정과 GT 일치 ≥ 9/10.
  - W4 동쪽 회귀 589·590 둘 다 성공(v7 동쪽 결과와 같은 계열).
  - W5 로봇–상대 접촉 0 step. 실패는 모두 깨끗한 중단(IN_SLOT 주장 없음)이어야 한다.
  - W6 계약: `validate_outcome`, `counts_as_m1 = false`, `cameras_seen = [robot_cam]`, 관측 거부 0, 소스 변경 없음, 깨끗한 시작. 등록 시드 10개가 모두 있다.
