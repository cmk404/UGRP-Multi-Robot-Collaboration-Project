...
2026-09-23 관리 통합: 아래는 최초 연결 당시의 검토 기록이다. 현재 실행 진입점과 기록 방식은 [표준 시뮬레이션 관리](simulation_management.md)를 따른다. 당시 19개 workflow를 실제 실행·기록·조회할 수 있게 연결했고, 출하장 실행기의 XML/초기화는 표준 `Scene`을 공유한다. 다른 호환 실행기의 제어 루프까지 재작성한 변경은 아니다. 이후 ACT 지도 준비와 입력·이력형 ACT 학습을 등록해 현재 카탈로그는 21개다.
...
| 기존 연구 스킬·학습·평가 경로를 새 사용자가 찾기 어려움 | 최초 workflows에서 19개 주요 진입점·필요 자료·범위 제공; 후속 2개 추가 |
