"""Derived TensorBoard view for the v3 cohort (+ v3 dev, v2 baseline, grip-hold probe).
...
the exported runs to short names. Usage:
  python experiments/2026-09-25-zone-owncam-skill/build_tensorboard_v3.py <cohort-dir-name> <snapshot-name>
"""
...
def main():
    ...
    runs = []
    for arm, profile in (('P', 'cargo_noslip_v1'), ('S', 'local_contact_fine')):
        for s in range(521, 531):
            src = RAW / cohort / arm / str(s)
            if (src / 'result.json').exists():
                runs.append(derived_run(src, f'v3{arm}-s{s}', f'{arm} {profile}',
                                        f'v3 pre-registered test arm {arm} ({profile}), source {cohort[-7:]}'))
    for src in sorted((RAW / 'dev-v3').glob('*/result.json')):
        seed, prof, _ = src.parent.name.split('-')
        runs.append(derived_run(src.parent, f'v3dev-{seed}-{prof}', f'dev {prof}',
                                'v3 development (not a result), 5117fa8'))
    for s in range(511, 521):
        src = RAW / 'cohort-v2-edd075d' / str(s)
        runs.append(derived_run(src, f'v2-s{s}', 'v2 local_contact_fine',
                                'v2 baseline test (edd075d), different scenarios'))
    target = TB / snapshot
    if target.exists():
