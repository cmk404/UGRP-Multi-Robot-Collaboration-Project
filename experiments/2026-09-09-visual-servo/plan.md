...
- e82098a r1/48: 25 local image transitions, 11 local-model actions. Of9 valid next same-view observations,3 reduced error and6 increased it. Output-only base displacement29.4cm; no close/contact. Exact48-request/action replay passed. This establishes exercised learning, not reliable control.
- Next candidate adds verified LK tracking/fusion (forward-back, texture and photometric checks; no hidden-point invention), reversal after observed error growth, and base repositioning after stalled alignment. Offline replay of e82098a images produced34 fused +10 bridged +2 model +2 unavailable observations,45 confidence-qualified geometry samples; this is tracking availability, not landmark accuracy. Candidate validation begins with r1/48 again, then r3 and pair only when meaningful.
