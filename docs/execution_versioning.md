...
- `rgb-standard-dispatch-v30`: 부모는 main의 v27이다(v28·v29는 별도 draft PR #129·#131이 사용 중이라 번호를 건너뛴다). 명시적 `--fine-gain-schedule`에서만, pose 영상 모델의 미세 yaw/측면/전진 단계 오차가 기존 허용오차의 3배를 넘으면 같은 영상 추정 오차의 약35%를 한 명령 구간에 줄이도록 모델 명령을 올린다. 모델 명령을 낮추거나 뒤집지 않고, 기록된 상위 사분위 응답에서도 바깥 허용오차를 넘지 않게 제한하며 기존 명령 상한을 유지한다. 준비 판정·허용오차·정지 확인·최종 정렬 재확인·dock·lease·예산은 같다. 판단마다 모델 명령과 스케줄을 기록한다. 기본값은 OFF이며 물리 완주·시간은 별도 검증한다.
