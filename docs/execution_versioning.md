...
- `rgb-standard-dispatch-v56`: 부모는 main의 v54다(v54·v51은 바이트 그대로 보존, v55는 #148이 사용 중). 명시적 `--coordination dynamic`의 복구 후진에서 coarse 바퀴 추적 영역을 후진 전 정지 TOP 영상 한 번과, 20 슬라이스 후진을 5 슬라이스씩 4번 나눈 각 묶음 뒤마다 다시 맞춘다. v51 실측 E2에서 미세 정렬 실패 뒤 한 번에 20 슬라이스를 후진하자 두 로봇이 약 64 px(추적 영역 반폭 55 px 초과) 이동해 r1 바퀴 픽셀이 76개(기준 80개 미만)로 줄었고, 다음 접근이 `coarse RGB model convention unresolved`로 멈췄다. 기본 `plan_first`와 다른 기본 OFF 옵션은 바뀌지 않는다. 물리 미검증이며 v51·v54 결과를 승계하지 않는다.
