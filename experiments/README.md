...
- [2026-09-26 구역 대화 연구 통합 러너 배선 스모크 (로드맵 8, #223)](2026-09-26-zone-study-integration/README.md): **임시, 표식 사용, 연구 결과 아님.** 연구 뼈대(#194)가 자기 카메라 실행기(#206) 3대를 한 러너에서 구동한다(no-LLM fixture, 자세 제공자 `tags_temporary`). 로봇 간 채널은 조건의 대화 채널과 짝 상태 채널(4조건 동일)뿐이며, 동료 비공개 상태 격리 검사와 변이 8/8로 확인했다. 주 4조건 × seed 700 물리 스모크: v1(`45999d9c`)은 사전 등록 게이트 P1–P8을 통과했으나 재질문 타이머 연쇄로 HTTP 90/90을 소진했다. v2(`9f7b16f2`)에서 이를 고쳤다. 배송은 두 버전·4조건 모두 2/3(r3 `SEARCH_NOT_FOUND`)이고 거짓 확인은 0이다. 조건 비교는 하지 않는다.
- [2026-09-26 구역 대화 연구 no-LLM 오프라인 스모크](2026-09-26-zone-study-offline-smoke/README.md): 연구 패키지 A(계약)·C(프로토콜)·D(SIM 비용)·E(시나리오)·I(평가)를 한 브랜치로 통합하고 저장 손목 RGB 참조 + 가짜 시계로 6 시나리오 × (주 4조건 + 참조 상한 R) × seed 1 = 30회를 돌렸다(`d87e0f2`, 모델 호출 0회·물리 0회). 30/30에서 채널 격리(`no_comm` 송수신 0, `leader_ko` follower끼리 0, `structured` 자유 문장 0, `peer_ko` 한국어 18/18)·비용 회계(호출 비용 = `released−requested`, 합계 = 스케줄러 사고 시간)·평가 역류 없음(비공개 구역만 바꾼 재실행에서 SIM trace·로그·입력 해시 동일 30/30)을 확인했다. 지휘자는 r1/r2/r3를 모두 덮는다. **프로토콜 배선만 검증했고 언어 이해·통신 효과·물리 운반 성공은 입증하지 않는다.** 물리가 없어 어떤 시행도 성공으로 집계되지 않는다(전 조건 0/6, censored 6/6).
  - [v2](2026-09-26-zone-study-offline-smoke/v2/README.md): Codex 적대적 검토 18건(입력 경계 우회·C–D 전달 인터페이스 불일치·비용 누락·조건별 평가 편향)을 수정한 뒤 같은 규모로 재실행했다(`dd497c8`, v1 결과 보존). 30/30 통과. 발화·전달 edge·SIM 비용은 v1과 같고, 대화 18회의 trace 해시만 canonical 메시지 ID 도입으로 달라졌다(ID를 가린 trace는 동일). 주문서 `scenario_id`가 불투명 ref로 바뀌어 주문서·입력 해시가 v1과 다르다. HTTP attempt 예약, 청구/생성 발화 일치, censored 호출(이 horizon에서는 0건), 조건별 고정 프롬프트 토큰을 새로 기록한다.
  - [v3](2026-09-26-zone-study-offline-smoke/v3/README.md): Codex 3차 재검토의 부분 해결 4건을 고친 뒤 재실행했다(`eb5572b`, v1·v2 보존). 고친 항목은 요청 원문 저장·디스크 재해시, fungible 복구 이력, order·kind·개체 주장 정규화, 사용량 미상 플래그 보존이다. 30/30 통과했고 저장 기록 30건을 다시 열어 요청 2168/2168건을 재해시했다. 3차 수정 전 코드 `02114ad`와 비교하면 30/30 시행에서 SIM trace·입력 해시·비용·채널이 같다. v2와 달라진 호출 수·비용은 2차 수정(`02114ad`)의 실제 토큰 청구에서 왔다.
  - [v4](2026-09-26-zone-study-offline-smoke/v4/README.md): Codex 4차 재검토 #16의 남은 반례 3건을 고친 뒤 재실행했다(`c21a6fe9`, v1~v3 보존). 반례는 재시도 예산 거절 때 미상 플래그 소실, censor 때 알려진 토큰 하한 소실, 보고서·TensorBoard의 미상 표지 누락이다. 회귀 테스트 7건은 수정 전 코드에서 7/7 실패하고, 변이 18개도 모두 이 테스트에서 걸렸다. `smoke.json`은 v3와 바이트 단위로 같다. 이 코호트에는 미상 호출이 0건이라 하한이 확정 합계와 같다. 처음으로 보고서와 TensorBoard 스냅샷(run 35개)을 만들었다. 모델 호출 0회·물리 0회.
- [2026-09-26 자기 손목 카메라 인식 v2: 동료 판별·팀 화물 부분 가시·들고 있는 can (패키지 B 후속)](2026-09-26-zone-own-perception-v2/README.md): v1 판단 4종을 한 줄도 고치지 않고 추가한 판단 4종 — 동료 MasterPi와 미등록 물체 구분(외관만), `long_beam`·`heavy_crate`·`tri_frame`의 종류와 “내 손잡이가 여기”, held-check 관측 자세의 들린 물건. CARRY가 담지 못하는 들린 can을 담는 자세를 측정으로 고르고(영역 안 0 → 73,657 px) 그 자세를 지나는 retention을 `cargo_noslip_v1`(최대 0.044 mm)과 `local_contact_fine`(0.485 mm)에서 weld OFF로 확인했다. dev/test 각 4시드·108뷰, 라벨은 평가 전용, 사전 등록 게이트 뒤 test 1회 채점. 제어 미연결.
- [2026-09-26 자기 손목 카메라 인식 판단 4종 (패키지 B)](2026-09-26-zone-own-perception/README.md): TOP이 평가 전용이 된 뒤 필요한 손목 어안 RGB 전용 판단 4종(pickup 슬롯 물건, 들고 있는 물건, 구역 슬롯 배치 다시 보기, 지도에 없는 전방 막힘). 모두 `yes`/`no`/`unknown`+신뢰도이고 `unknown`은 1급 답이다. dev 4시드·test 4시드(분할당 92뷰), 정답 교사는 데이터 생성·채점 전용, 위치 belief는 주입값 3단계. dev 확정 뒤 test 1회 채점. 사전 등록한 임계값·게이트는 기록의 표에 있다. 제어 미연결.
